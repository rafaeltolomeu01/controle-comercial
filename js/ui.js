const UI = {
    ...
    listBody.innerHTML = clients.map(client => {
      ...
      return `
        <tr class="mobile-summary-row" onclick="App.showClientDetails('${client.id}')">
          ...
          <td>${client.score != null ? client.score : '-'} (${client.classification || '-'})</td>
    ...
    listBody.innerHTML = movements.map(mov => {
      ...
      return `
        <tr class="mobile-summary-row" onclick="App.showMovementDetails('${mov.id}')">
          ...
          <td style="font-weight:600;">${mov.cliente_nome || '-'}</td>
          <td>${mov.cliente_cidade || '-'}</td>
          <td>${mov.vendedor_solicitante || '-'}</td>
          <td style="font-family:monospace; font-size:0.75rem;">${patrimonioStr}</td>
          <td>${modeloStr}</td>
    ...
    listBody.innerHTML = expenses.map(exp => {
      ...
      return `
        <tr class="mobile-summary-row" onclick="App.generateExpenseComprovantePdf('${exp.id}')">
          ...
          <td class="normal-wrap">${finalidadeStr}</td>
          <td>${exp.operacao || ''}</td>
  ...
  showClientDetails(client) {
    ...
    // Helper to resolve photos with fallbacks
    const resolvePhotoUrl = (mockUrl, type) => {
      if (window.TempPhotosCache && window.TempPhotosCache[mockUrl]) {
        return window.TempPhotosCache[mockUrl];
      }
      const fallbacks = {
        fachada: 'https://images.unsplash.com/photo-1578916171728-46686eac8d58?w=800&auto=format&fit=crop&q=80',
        interna01: 'https://images.unsplash.com/photo-1563245372-f21724e3856d?w=800&auto=format&fit=crop&q=80',
        interna02: 'https://images.unsplash.com/photo-1542838132-92c53300491e?w=800&auto=format&fit=crop&q=80',
        interna03: 'https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?w=800&auto=format&fit=crop&q=80',
        rua01: 'https://images.unsplash.com/photo-1569336415962-a4bd9f69cd83?w=800&auto=format&fit=crop&q=80',
        rua02: 'https://images.unsplash.com/photo-1507035895480-2b3156c31fc8?w=800&auto=format&fit=crop&q=80',
        cnpj: 'https://images.unsplash.com/photo-1450133064473-71024230f91b?w=800&auto=format&fit=crop&q=80'
      };
      return fallbacks[type] || 'https://images.unsplash.com/photo-1450133064473-71024230f91b?w=800&auto=format&fit=crop&q=80';
    };
    ...
    content.innerHTML = `
      ...
      <div style="background: rgba(255,255,255,0.01); border: 1px solid var(--border-color); border-radius: 8px; padding: 14px;">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 12px;">
          <!-- 1. Fachada -->
          <div style="display: flex; flex-direction: column; align-items: center; border: 1px solid var(--border-color); padding: 8px; border-radius: 6px; background: rgba(0,0,0,0.2);">
            <span style="font-size: 0.68rem; font-weight: bold; margin-bottom: 6px; color: var(--text-muted);">Fachada</span>
            <img src="${photoFachadaUrl}" style="width: 100%; height: 110px; object-fit: cover; border-radius: 4px; cursor: pointer; transition: transform 0.2s;" onclick="App.showFacadeImage('${photoFachadaUrl.replace(/'/g, "\\'")}')" title="Clique para ampliar">
            <span style="font-size: 0.58rem; color: var(--text-muted); text-align: center; margin-top: 6px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; width: 100%;" title="${client.photoFachada || 'Link indisponível'}">${client.photoFachada || 'Link simulado'}</span>
          </div>

          <!-- 2. Interna 01 -->
          <div style="display: flex; flex-direction: column; align-items: center; border: 1px solid var(--border-color); padding: 8px; border-radius: 6px; background: rgba(0,0,0,0.2);">
            <span style="font-size: 0.68rem; font-weight: bold; margin-bottom: 6px; color: var(--text-muted);">Interna 01</span>
            <img src="${photoInterna01Url}" style="width: 100%; height: 110px; object-fit: cover; border-radius: 4px; cursor: pointer; transition: transform 0.2s;" onclick="App.showFacadeImage('${photoInterna01Url.replace(/'/g, "\\'")}')" title="Clique para ampliar">
            <span style="font-size: 0.58rem; color: var(--text-muted); text-align: center; margin-top: 6px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; width: 100%;" title="${client.photoInterna01 || 'Link indisponível'}">${client.photoInterna01 || 'Link simulado'}</span>
          </div>

          <!-- 3. Interna 02 -->
          <div style="display: flex; flex-direction: column; align-items: center; border: 1px solid var(--border-color); padding: 8px; border-radius: 6px; background: rgba(0,0,0,0.2);">
            <span style="font-size: 0.68rem; font-weight: bold; margin-bottom: 6px; color: var(--text-muted);">Interna 02</span>
            <img src="${photoInterna02Url}" style="width: 100%; height: 110px; object-fit: cover; border-radius: 4px; cursor: pointer; transition: transform 0.2s;" onclick="App.showFacadeImage('${photoInterna02Url.replace(/'/g, "\\'")}')" title="Clique para ampliar">
            <span style="font-size: 0.58rem; color: var(--text-muted); text-align: center; margin-top: 6px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; width: 100%;" title="${client.photoInterna02 || 'Link indisponível'}">${client.photoInterna02 || 'Link simulado'}</span>
          </div>

          <!-- 4. Interna 03 -->
          <div style="display: flex; flex-direction: column; align-items: center; border: 1px solid var(--border-color); padding: 8px; border-radius: 6px; background: rgba(0,0,0,0.2);">
            <span style="font-size: 0.68rem; font-weight: bold; margin-bottom: 6px; color: var(--text-muted);">Interna 03</span>
            <img src="${photoInterna03Url}" style="width: 100%; height: 110px; object-fit: cover; border-radius: 4px; cursor: pointer; transition: transform 0.2s;" onclick="App.showFacadeImage('${photoInterna03Url.replace(/'/g, "\\'")}')" title="Clique para ampliar">
            <span style="font-size: 0.58rem; color: var(--text-muted); text-align: center; margin-top: 6px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; width: 100%;" title="${client.photoInterna03 || 'Link indisponível'}">${client.photoInterna03 || 'Link simulado'}</span>
          </div>

          <!-- 5. Rua 01 -->
          <div style="display: flex; flex-direction: column; align-items: center; border: 1px solid var(--border-color); padding: 8px; border-radius: 6px; background: rgba(0,0,0,0.2);">
            <span style="font-size: 0.68rem; font-weight: bold; margin-bottom: 6px; color: var(--text-muted);">Externa Rua 01</span>
            <img src="${photoRua01Url}" style="width: 100%; height: 110px; object-fit: cover; border-radius: 4px; cursor: pointer; transition: transform 0.2s;" onclick="App.showFacadeImage('${photoRua01Url.replace(/'/g, "\\'")}')" title="Clique para ampliar">
            <span style="font-size: 0.58rem; color: var(--text-muted); text-align: center; margin-top: 6px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; width: 100%;" title="${client.photoRua01 || 'Link indisponível'}">${client.photoRua01 || 'Link simulado'}</span>
          </div>

          <!-- 6. Rua 02 -->
          <div style="display: flex; flex-direction: column; align-items: center; border: 1px solid var(--border-color); padding: 8px; border-radius: 6px; background: rgba(0,0,0,0.2);">
            <span style="font-size: 0.68rem; font-weight: bold; margin-bottom: 6px; color: var(--text-muted);">Externa Rua 02</span>
            <img src="${photoRua02Url}" style="width: 100%; height: 110px; object-fit: cover; border-radius: 4px; cursor: pointer; transition: transform 0.2s;" onclick="App.showFacadeImage('${photoRua02Url.replace(/'/g, "\\'")}')" title="Clique para ampliar">
            <span style="font-size: 0.58rem; color: var(--text-muted); text-align: center; margin-top: 6px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; width: 100%;" title="${client.photoRua02 || 'Link indisponível'}">${client.photoRua02 || 'Link simulado'}</span>
          </div>

          <!-- 7. CNPJ -->
          <div style="display: flex; flex-direction: column; align-items: center; border: 1px solid var(--border-color); padding: 8px; border-radius: 6px; background: rgba(0,0,0,0.2);">
            <span style="font-size: 0.68rem; font-weight: bold; margin-bottom: 6px; color: var(--text-muted);">Foto CNPJ</span>
            <img src="${photoCnpjUrl}" style="width: 100%; height: 110px; object-fit: cover; border-radius: 4px; cursor: pointer; transition: transform 0.2s;" onclick="App.showFacadeImage('${photoCnpjUrl.replace(/'/g, "\\'")}')" title="Clique para ampliar">
            <span style="font-size: 0.58rem; color: var(--text-muted); text-align: center; margin-top: 6px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; width: 100%;" title="${client.photoCnpj || 'Link indisponível'}">${client.photoCnpj || 'Link simulado'}</span>
          </div>
        </div>
      </div>
    `;
