...
/* ============================================================
   AtualizaÃ§Ãµes 01-13 - CorreÃ§Ãµes pontuais sem alterar estrutura
   ============================================================ */
(function(){
  ...
  // 01 - Limpa dados temporÃ¡rios de formulÃ¡rios por login/rota/envio.
  ...
  // 02 - PWA instalÃ¡vel (registro seguro do service worker existente).
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('/sw.js').catch(()=>{}));
  }

  // 07 - PermissÃµes estritas: se houver checkboxes salvos, nÃ£o liberar mÃ³dulos por perfil.
  if (window.Store) {
    Store.getUserAllowedRoutes = function(user){
      ...
      if (perms.length > 0) {
        ...
        if (perms.includes('ProspecÃ§Ã£o') || perms.includes('ProspecÃ§Ã£o (Leads)')) add(allowed, '#prospeccao');
        ...
        if (perms.includes('Chamados') || perms.includes('Chamados MecÃ¢nicos')) add(allowed, '#chamados');
        if (perms.includes('Despesas') || perms.includes('Despesas de Campo')) add(allowed, '#despesas');
        if (perms.includes('SolicitaÃ§Ã£o de Saldo')) add(allowed, '#solicitacao-despesas','#despesas');
        if (perms.includes('AprovaÃ§Ã£o de Saldo') || perms.includes('AprovaÃ§Ã£o de Despesas') || perms.includes('Financeiro')) add(allowed, '#despesas-dashboard','#despesas','#solicitacao-despesas');
        if (perms.includes('Simulador de Troca')) add(allowed, '#simulador-troca');
        if (perms.includes('UsuÃ¡rios') || perms.includes('UsuÃ¡rios e PermissÃµes')) add(allowed, '#usuarios');
        if (perms.includes('ConfiguraÃ§Ãµes') || perms.includes('ConfiguraÃ§Ãµes Gerais')) add(allowed, '#configuracoes','#empresa','#unidades');
        return allowed;
      }
      ...
      if (user.profile === 'MecÃ¢nico') add(allowed,'#chamados');
      if (user.profile === 'ResponsÃ¡vel Equipamentos') add(allowed,'#equipamentos','#movimentacao','#chamados');
      return allowed;
    };
  ...
  // 08 - Dropdowns de empresas/unidades somente com unidades oficiais e conforme vÃ­nculo.
  if (window.UI) {
    UI.populateUnitDropdowns = async function(){
      ...
      // vendedores/solicitantes sÃ³ da unidade permitida
      try {
  ...
  // 09/10 - ConfiguraÃ§Ã£o de hospedagem por unidade e perfil.
  function ensureUnitFinanceFields(){
    ...
    block.innerHTML = `
      <h4 style="margin:0 0 12px;color:var(--primary-color);">ConfiguraÃ§Ãµes Financeiras - Hospedagem</h4>
      <div class="form-row">
        <div class="form-group"><label>DiÃ¡ria Vendedor (R$)</label><input type="number" id="unit-daily-vendedor" step="0.01" min="0" value="120"></div>
        <div class="form-group"><label>DiÃ¡ria Supervisor (R$)</label><input type="number" id="unit-daily-supervisor" step="0.01" min="0" value="150"></div>
        <div class="form-group"><label>DiÃ¡ria Gerente (R$)</label><input type="number" id="unit-daily-gerente" step="0.01" min="0" value="180"></div>
      </div>
      <div class="form-row">
        <div class="form-group"><label>MÃ¡ximo de DiÃ¡rias</label><input type="number" id="unit-max-nights" min="1" step="1" value="4"></div>
        <div class="form-group"><label>Permitir Sem Hospedagem</label><select id="unit-allow-no-hotel"><option value="true">Sim</option><option value="false">NÃ£o</option></select></div>
      </div>`;
    form.appendChild(block);
  }
  window.CC_ensureUnitFinanceFields = ensureUnitFinanceFields;

  // Override renderUnits para garantir bloco financeiro e botÃ£o editar.
  ...
  // Enriquecer salvamento/ediÃ§Ã£o de unidade sem mexer no restante.
  setTimeout(() => {
    ensureUnitFinanceFields();
  ...
  if (window.App) {
    ...
    // 08/09/10 - SolicitaÃ§Ã£o de saldo usa empresa oficial e diÃ¡ria por unidade/perfil.
    ...
    if (oldSubmitSol) App.submitSolicitacaoDespesas = async function(){
      // substitui radios fixos antes da funÃ§Ã£o original calcular
      const unit = getCurrentUnit();
  ...
  // 03/04/05/06 - ExclusÃ£o em massa, somente Admin. ImplementaÃ§Ã£o segura para principais listas locais/API.
  function addBulkForTable(tbodyId, storeGetter, storeSaver, label){
    ...
    btn.onclick = async () => {
      ...
      if (!confirm('Tem certeza que deseja excluir os registros selecionados? Esta aÃ§Ã£o nÃ£o poderÃ¡ ser desfeita.')) return;
      try {
  ...
  function enhanceBulkDeletes(){
    ...
    // addBulkForTable('movements-table-body','getMovements','saveMovements','movimentaÃ§Ãµes');
    ...
    addBulkForTable('users-table-body','getUsers','saveUsers','usuÃ¡rios');
    addBulkForTable('units-table-body','getUnits','saveUnits','unidades');
    addBulkForTable('despesas-solicitacoes-table-body',null,null,'solicitaÃ§Ãµes');
    // ProspecÃ§Ã£o usa container customizado
  ...
  // 06 - Oculta botÃµes individuais de excluir para nÃ£o administradores.
  ...
  // 11 - Corrige .toFixed em valores string na aprovaÃ§Ã£o de saldo.
  ...
  // 12/13 - PDFs: emissÃ£o vÃ¡lida e empresa correta.
  ...
  window.CC_pdfCompanyName = function(record){
    record = record || {};
    if (record.empresa && !/nÃ£o informada/i.test(record.empresa)) return record.empresa;
    const unit = getUnitById(record.unitId || record.unit_id || record.empresa_id) || getCurrentUnit();
    return unit.name || 'NÃ£o informada';
  };
...
/* Rodada 14-19 - correÃ§Ãµes pontuais sem alterar fluxos existentes */
(function(){
  'use strict';
  const money = (v) => new Intl.NumberFormat('pt-BR',{style:'currency',currency:'BRL'}).format(Number(v)||0);
  ...
  const canApproveExpenses = () => { const u = Store.getLoggedUser() || {}; const p = u.permissions || []; return isAdmin() || u.profile === 'Financeiro' || p.includes('Financeiro') || p.includes('AprovaÃ§Ã£o de Despesas'); };
  ...
  function ensureUnitFinanceFields(){
    ...
    block.innerHTML = `
      <h3 style="color:var(--primary-color); font-size:1rem; margin-bottom:14px;">ConfiguraÃ§Ãµes Financeiras - Hospedagem</h3>
      <div class="form-row">
        <div class="form-group"><label for="unit-diaria-vendedor">DiÃ¡ria Vendedor (R$)</label><input type="number" id="unit-diaria-vendedor" min="0" step="0.01" value="120"></div>
        <div class="form-group"><label for="unit-diaria-supervisor">DiÃ¡ria Supervisor (R$)</label><input type="number" id="unit-diaria-supervisor" min="0" step="0.01" value="150"></div>
        <div class="form-group"><label for="unit-diaria-gerente">DiÃ¡ria Gerente (R$)</label><input type="number" id="unit-diaria-gerente" min="0" step="0.01" value="180"></div>
      </div>
      <div class="form-row">
        <div class="form-group"><label for="unit-maximo-diarias">MÃ¡ximo de DiÃ¡rias</label><input type="number" id="unit-maximo-diarias" min="0" step="1" value="4"></div>
        <div class="form-group"><label for="unit-permitir-sem-hospedagem">Permitir Sem Hospedagem</label><select id="unit-permitir-sem-hospedagem"><option value="true">Sim</option><option value="false">NÃ£o</option></select></div>
      </div>`;
    form.appendChild(block);
  }
  ...
  // 16/17/18 - Unidades: salvar config financeira, ID sequencial e excluir unidade com proteÃ§Ã£o simples.
  ...
  function bindUnitFormPatch(){
    ...
    form.addEventListener('submit', (e)=>{
      ...
      App.showToast?.(editingId ? 'ConfiguraÃ§Ãµes da unidade atualizadas com sucesso.' : 'Unidade cadastrada com sucesso.');
    }, true);
  }
  ...
  App.deleteUnit = function(unitId){
    ...
    if (hasLinks) return alert('NÃ£o Ã© possÃ­vel excluir esta empresa porque existem registros vinculados a ela.');
    if (!confirm('Tem certeza que deseja excluir esta empresa? Essa aÃ§Ã£o nÃ£o poderÃ¡ ser desfeita.')) return;
    Store.saveUnits(units.filter(u=>String(u.id)!==String(unitId)));
    UI.populateUnitDropdowns?.(); UI.renderUnits?.(); App.showToast?.('Empresa excluÃ­da com sucesso.');
  };

  // 14/15 - AprovaÃ§Ã£o de Despesas de Campo + imagens no PDF.
  ...
  async function setExpenseStatus(id, status){
    let observacao = '';
    if (status !== 'Aprovado') {
      observacao = prompt(status === 'CorreÃ§Ã£o Solicitada' ? 'Informe o que precisa ser corrigido:' : 'Informe o motivo da reprovaÃ§Ã£o:') || '';
      if (!observacao.trim()) return alert('Motivo obrigatÃ³rio.');
    }
  ...
  App.correctRegisteredExpense = (id)=>setExpenseStatus(id,'CorreÃ§Ã£o Solicitada');
  ...
  App.generateExpenseComprovantePdf = async function(id){
    try {
      ...
      if (!exp) return this.showToast?.('Despesa nÃ£o encontrada!', 'danger');
      ...
      doc.text(`Comprovante ID: #${exp.id}`,15,33); doc.text(`Data / Hora: ${safeDate(exp.date)}${exp.time ? ' Ã s '+exp.time : ''}`,110,33);
      doc.text(`Vendedor: ${UI.getExpenseUserName(exp)}`,15,39); doc.text(`Unidade: ${UI.getUnitName(exp.unitId)}`,110,39);
      doc.text(`Finalidade: ${exp.finalidade || '-'}`,15,45); doc.text(`Tipo de OperaÃ§Ã£o: ${exp.operacao || '-'}`,110,45); doc.text(`Status: ${exp.status || '-'}`,15,51);
      ...
      if (exp.observation) { const lines=doc.splitTextToSize(`ObservaÃ§Ã£o: ${exp.observation}`,180); doc.text(lines,15,y); y+=lines.length*5+4; }
      if (exp.descreva) { const lines=doc.splitTextToSize(`DescriÃ§Ã£o: ${exp.descreva}`,180); doc.text(lines,15,y); y+=lines.length*5+4; }
      doc.line(10,y,200,y); y+=8; doc.setFont('Helvetica','bold'); doc.text('ANEXOS / COMPROVANTES FOTOGRÃFICOS',15,y); y+=8; doc.setFont('Helvetica','normal');
      const imgs = [];
      if (exp.foto_comprovante) imgs.push(['Comprovante', exp.foto_comprovante]);
      if (exp.foto_odometro) imgs.push(['OdÃ´metro / KM', exp.foto_odometro]);
  ...
  // 19 - Datas vÃ¡lidas em solicitaÃ§Ã£o/aprovaÃ§Ã£o de saldo.
  ...
  // Abas visuais de aprovaÃ§Ã£o de despesas.
  function ensureExpenseApprovalTab(){
    ...
    const a = document.createElement('a'); a.href='#despesas'; a.id='tab-expense-approvals'; a.className='view-tab-btn'; a.textContent='AprovaÃ§Ã£o de Despesas';
...
/* Rodada 20-25 - ajustes pontuais em saldo, aprovaÃ§Ã£o de despesas, permissÃµes e dashboard */
(function(){
  'use strict';
  const money = (v) => new Intl.NumberFormat('pt-BR',{style:'currency',currency:'BRL'}).format(Number(v)||0);
  const num = (v) => {
  ...
  const canApproveExpense = (u=getUser()) => isAdmin(u) || u.profile === 'Financeiro' || perms(u).includes('Financeiro') || perms(u).includes('AprovaÃ§Ã£o de Despesas');
  const canApproveBalance = (u=getUser()) => isAdmin(u) || u.profile === 'Financeiro' || perms(u).includes('Financeiro') || perms(u).includes('AprovaÃ§Ã£o de Saldo');
  ...
  // PermissÃµes: manter padrÃ£o existente, mas impedir AprovaÃ§Ã£o de Despesas para quem nÃ£o tem permissÃ£o explÃ­cita/perfil financeiro/admin.
  ...
  function ensureTabs(){
    ...
    const tabs = [
      ...
      ['#solicitacao-despesas','SolicitaÃ§Ã£o de Saldo','tab-balance-solicitation', (Store.getUserAllowedRoutes(getUser())||[]).includes('#solicitacao-despesas')],
      ['#despesas-dashboard','AprovaÃ§Ã£o de Saldo','tab-balance-approvals', canApproveBalance()],
      ['#despesas','AprovaÃ§Ã£o de Despesas','tab-expense-approvals', canApproveExpense()]
    ];
  ...
  function ensureChartsHost(){
    ...
    box.innerHTML = `<div class="card-header"><span class="card-title">GrÃ¡ficos Financeiros</span></div><div id="finance-charts-body" style="display:grid; grid-template-columns:repeat(auto-fit,minmax(240px,1fr)); gap:16px;"></div>`;
  ...
  function renderFinanceCharts(ctx){
    ...
    body.innerHTML = `
      <div><h4>Resumo por Status</h4>${bar('Saldo aprovado',ctx.approvedBalance,max)}${bar('Despesas aprovadas',ctx.approvedExpense,max)}${bar('Saldo restante',ctx.remaining,max)}${bar('Pendentes',pendingExp,max)}${bar('CorreÃ§Ã£o',corrExp,max)}${bar('Reprovadas',rejExp,max)}</div>
      <div><h4>Pizza por situaÃ§Ã£o</h4><div style="min-height:160px; border-radius:12px; background:conic-gradient(var(--success) 0 ${Math.round((ctx.approvedExpense/(max||1))*360)}deg, var(--warning) 0 ${Math.round(((ctx.approvedExpense+pendingExp)/(max||1))*360)}deg, var(--danger) 0); display:flex; align-items:center; justify-content:center;"><span style="background:var(--bg-card); padding:14px; border-radius:999px; font-weight:700;">${money(ctx.remaining)}</span></div></div>
      <div><h4>Ranking / Unidade</h4><ol style="padding-left:18px; margin:0;">${unitRows}</ol></div>`;
  }

  // AprovaÃ§Ã£o de despesas registradas: linha abre detalhes; aÃ§Ãµes sÃ³ dentro do modal.
  const oldRenderExpenses = UI.renderExpenses?.bind(UI);
  UI.renderExpenses = function(expenses){
    oldRenderExpenses?.(expenses);
    const body = document.getElementById('expenses-table-body'); if (!body) return;
  ...
  App.showRegisteredExpenseDetails = async function(id){
    try{
      const exp = await this.fetchFromApi(`/api/despesas-reembolsos/${id}`);
      if (!exp) return alert('Despesa nÃ£o encontrada.');
  ...
  App.showDespesaDetails = async function(id){
    ...
    setTimeout(()=>{
      document.querySelectorAll('.approval-item-row').forEach(row=>{
        ...
        if (!sel || !val) return;
        if (sel.querySelector('option[value="correcao"]') == null) sel.insertAdjacentHTML('beforeend','<option value="correcao">Enviar para CorreÃ§Ã£o</option>');
        ...
        const apply = () => {
          ...
          if (just) just.placeholder = partial ? 'Justificativa obrigatÃ³ria...' : 'Opcional para aprovaÃ§Ã£o total';
        ...
        const label = just?.closest('.form-group')?.querySelector('label'); if (label) label.textContent = 'Justificativa (obrigatÃ³ria para valor/quantidade menor, reprovaÃ§Ã£o ou correÃ§Ã£o)';
      });
    },120);
  };
  ...
  App.submitExpenseApproval = async function(){
    ...
    document.querySelectorAll('.approval-item-row').forEach(row=>{
      ...
      if (needs && !justificativa) error = 'Justificativa obrigatÃ³ria para valor/quantidade menor, reprovaÃ§Ã£o ou envio para correÃ§Ã£o.';
      if (valApproved > originalVal) error = 'O valor aprovado nÃ£o pode ser maior que o valor solicitado.';
      items.push({
        id: itemId,
    ...
    try {
      ...
      if (result?.success) {
        this.showToast(`SolicitaÃ§Ã£o #${id} avaliada com sucesso!`);
  ...
  App.loadDespesasDashboard = async function(){
    ...
    } catch(e) { console.warn('NÃ£o foi possÃ­vel recalcular saldos filtrados.', e); }
...
/* CorreÃ§Ãµes 30/06 - saldo, datas, PDF e abas sem recriar mÃ³dulos */
(function(){
  ...
  window.CC_safeDateBR = function(value){
    ...
    // 1. JÃ¡ no formato BR: manter intacto
    ...
    // 3. Timestamp ISO completo com fuso horÃ¡rio (ex: Z, -03:00): converter para o fuso local do navegador
  ...
  function fixApprovalValues(){
    document.querySelectorAll('.approval-item-row').forEach(row=>{
      ...
      if (label && /Solicitado:/.test(label.textContent)) {
        ...
        label.textContent = `Solicitado: ${money(original)}${qtd ? ' (' + qtd + ' diÃ¡rias)' : ''}`;
      }
    });
    let total=0;
    document.querySelectorAll('.approval-item-row').forEach(row=>{
...
/* CorreÃ§Ãµes rodada atual 30/06 - abas, exclusÃµes, aprovaÃ§Ã£o de despesas e PDF em lote */
(function(){
  'use strict';
  ...
  const canApproveExpense = (u=user())=>isAdmin(u) || u.profile==='Financeiro' || perms(u).includes('Financeiro') || perms(u).includes('AprovaÃ§Ã£o de Despesas');

  function apiDelete(url){
    if (!window.App?.fetchFromApi) return Promise.resolve();
    return App.fetchFromApi(url,{method:'DELETE'}).catch(e=>{ console.warn('Falha no DELETE API, removendo local se possÃ­vel:', url, e); });
  }
  ...
  function cleanTabs(){
    ...
    document.querySelectorAll('.view-tabs').forEach(host=>{
      ...
      const sol = byText.get('SolicitaÃ§Ã£o de Saldo');
      const aprSaldo = byText.get('AprovaÃ§Ã£o de Saldo');
      const aprDesp = byText.get('AprovaÃ§Ã£o de Despesas');
  ...
  // AprovaÃ§Ã£o de despesas deve mostrar sÃ³ pendentes e sem formulÃ¡rio de registro.
    ...
    App.loadExpenses = async function(){
      ...
      setTimeout(()=>{
        ...
        if (mode) {
          ...
          const title = document.querySelector('#view-despesas h1, #view-despesas .page-title'); if (title) title.textContent='AprovaÃ§Ã£o de Despesas';
  ...
  // ExclusÃ£o individual real de leads com API + local.
  if (window.App && !App.__ccDeleteProspectAtual) {
    App.__ccDeleteProspectAtual = true;
    App.deleteProspectReal = async function(id){
      if(!isAdmin()) return alert('Somente administrador pode excluir registros.');
      if(!confirm('Deseja excluir permanentemente este lead de prospecÃ§Ã£o?')) return;
  ...
  // PDF individual: tabela com largura fixa e assinatura sem sobreposiÃ§Ã£o.
  if (window.App && !App.__ccPdfSaldoAtual) {
    ...
    App.generateBalanceRequestPdf = async function(id){
      try{
        ...
        const html = buildSaldoPdfHtml([data], rows, totalSol, totalApr, `SOLICITAÃ‡ÃƒO DE SALDO / ADIANTAMENTO DE SALDO`);
        const w=window.open('','_blank'); w.document.write(html); w.document.close();
      }catch(e){ alert('Erro ao gerar PDF: '+e.message); }
    };
  }
  ...
  // PDF em lote para SolicitaÃ§Ãµes de Saldo com filtros simples/dinÃ¢micos.
  ...
  if(window.App) App.generateBatchBalancePdf = async function(){
    try{
      ...
      if(!detailed.length) return alert('Nenhuma solicitaÃ§Ã£o encontrada para os filtros informados.');
      const w=window.open('','_blank'); w.document.write(buildSaldoPdfHtml(detailed,null,null,null,'RELATÃ“RIO EM LOTE - SOLICITAÃ‡Ã•ES DE SALDO')); w.document.close(); document.getElementById('modal-pdf-lote-saldo')?.remove();
...
/* CorreÃ§Ã£o definitiva 30/06 - abas de Despesas sem duplicar, sem piscar e com navegaÃ§Ã£o funcional */
(function(){
  'use strict';
  ...
  const TABS = [
    ...
    { id:'tab-balance-solicitation', label:'SolicitaÃ§Ã£o de Saldo', hash:'#solicitacao-despesas', mode:'' },
    { id:'tab-balance-approvals', label:'AprovaÃ§Ã£o de Saldo', hash:'#despesas-dashboard', mode:'' },
    { id:'tab-expense-approvals', label:'AprovaÃ§Ã£o de Despesas', hash:'#despesas', mode:'approval' }
  ];
  ...
  const canApproveExpense = () => {
    ...
    return u.profile === 'Administrador' || u.profile === 'Financeiro' ||
      ...
      p.includes('Financeiro') || p.includes('AprovaÃ§Ã£o de Despesas');
  };
  const canApproveBalance = () => {
    const {user:u, list:p} = perms();
    return u.profile === 'Administrador' || u.profile === 'Financeiro' ||
      p.includes('Administrador') || p.includes('Administrador (Acesso Total)') ||
      p.includes('Financeiro') || p.includes('AprovaÃ§Ã£o de Saldo');
  };
  const canSolicitBalance = () => {
    const {user:u, list:p} = perms();
    return u.profile === 'Administrador' || u.profile === 'Financeiro' ||
      p.includes('Administrador') || p.includes('Administrador (Acesso Total)') ||
      p.includes('Financeiro') || p.includes('SolicitaÃ§Ã£o de Saldo') || p.includes('Despesas') || p.includes('Despesas de Campo');
  };
  ...
  function ensureHost(host){
    ...
    try {
      ...
      TABS.forEach(tab => {
        ...
        // Remove listeners antigos uma Ãºnica vez.
      ...
      // Ordem fixa sem ficar reapendando os mesmos nÃ³s a cada MutationObserver.
  ...
  // NÃ£o observar o documento inteiro: isso fazia as abas serem reconstruÃ­das quando qualquer lista mudava.
  // Basta atualizar em carregamento, troca de rota e poucos cliques de navegaÃ§Ã£o.
...
/* CorreÃ§Ã£o final 30/06 - abas restritas ao mÃ³dulo correto + simulador de troca com ediÃ§Ã£o prÃ³pria */
(function(){
  'use strict';
  if (window.__ccTrocaAbasFinal3006) return;
  window.__ccTrocaAbasFinal3006 = true;

  const FINANCE_LABELS = ['Despesas de Campo','SolicitaÃ§Ã£o de Saldo','AprovaÃ§Ã£o de Saldo','AprovaÃ§Ã£o de Despesas'];
  const EXCHANGE_LABELS = ['Nova Troca','HistÃ³rico de Trocas','Importar Planilha'];
  const esc = (v)=>String(v ?? '').replace(/[&<>"']/g, ch => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch]));
  const money = (v)=>new Intl.NumberFormat('pt-BR',{style:'currency',currency:'BRL'}).format(Number(v)||0);
  ...
  function cleanExchangeTabs(){
    ...
    // Garante que sÃ³ as trÃªs abas prÃ³prias apareÃ§am e nÃ£o exista confirmaÃ§Ã£o/injeÃ§Ã£o financeira.
    Array.from(host.querySelectorAll('a,button')).forEach(el => {
      const label = (el.textContent || '').replace(/\s+/g,' ').trim();
  ...
  if (window.App && !App.__ccTrocaLoadHistory3006) {
    ...
    App.loadExchangeHistory = async function(){
      try {
        ...
        console.error('Erro ao buscar histÃ³rico de simulaÃ§Ãµes:', err);
      }
    };
  }
  ...
  if (window.UI && !UI.__ccTrocaRenderHistory3006) {
    ...
    UI.renderExchangeHistory = function(simulations){
      ...
      if (!list.length) {
        listContainer.innerHTML = '<div style="text-align:center; padding:20px; color:var(--text-muted); font-size:.85rem;">Nenhuma simulaÃ§Ã£o de troca encontrada.</div>';
        return;
      }
  ...
  if (window.App && !App.__ccTrocaEdit3006) {
    ...
    App.deleteExchangeSimulation = async function(id) {
      if (!confirm('Deseja realmente excluir permanentemente esta simulaÃ§Ã£o de troca?')) return;
      try {
        const res = await App.fetchFromApi(`/api/exchange/simulations/${encodeURIComponent(id)}`, {
          method: 'DELETE'
        });
        if (res && (res.success || res.ok)) {
          App.showToast && App.showToast('SimulaÃ§Ã£o de troca excluÃ­da com sucesso!');
          App.loadExchangeHistory && App.loadExchangeHistory();
        }
      } catch (err) {
        console.error(err);
        alert('Erro ao excluir simulaÃ§Ã£o: ' + (err.message || err));
      }
    };
    ...
    App.editExchangeSimulation = async function(id){
      try {
        ...
        if (!canEditSim(data)) return alert('VocÃª sÃ³ pode editar trocas que vocÃª mesmo cadastrou.');
        ...
        App.showToast && App.showToast('Troca carregada para ediÃ§Ã£o.');
      } catch (err) {
        console.error(err);
        alert('Erro ao carregar troca para ediÃ§Ã£o: ' + (err.message || err));
      }
    };
    ...
    App.finalizeExchange = async function(){
      ...
      if (!clientCode || !clientName) return alert('IdentificaÃ§Ã£o do cliente invÃ¡lida.');
      ...
      try {
        ...
        if (res && (res.success || res.id || res.ok)) {
          App.showToast && App.showToast(isEdit ? 'Troca atualizada com sucesso!' : 'LanÃ§amento da troca finalizado!');
        ...
        console.error(err);
        alert('Erro ao salvar simulaÃ§Ã£o: ' + (err.message || err));
      }
    };
  }
})();
...
/* AtualizaÃ§Ã£o automÃ¡tica do sistema apÃ³s deploy. */
(function () {
  'use strict';
...
/* CorreÃ§Ã£o 30/06 - AprovaÃ§Ã£o/ReprovaÃ§Ã£o de Despesas
   Ajuste pontual: corrige botÃµes do modal de despesa, fecha a tela apÃ³s concluir
   e evita que erro secundÃ¡rio deixe a interface travada. */
(function(){
  'use strict';
    ...
    for (const ep of endpoints) {
      try {
        ...
        if (result && (result.success === false || result.ok === false)) {
          lastError = new Error(result.error || 'A API recusou a avaliaÃ§Ã£o da despesa.');
          continue;
        }
  ...
  async function setExpenseStatusFixed(id, status){
    if (!id) return alert('Despesa nÃ£o identificada. Feche a tela e abra novamente.');

    let observacao = '';
    if (!isApproved(status)) {
      const label = norm(status).includes('correc') ? 'Informe o que precisa ser corrigido:' : 'Informe o motivo da reprovaÃ§Ã£o:';
      observacao = prompt(label) || '';
      if (!observacao.trim()) {
        alert('A observaÃ§Ã£o/motivo Ã© obrigatÃ³ria.');
        return;
      }
    }
    ...
    try {
      ...
      alert('NÃ£o foi possÃ­vel avaliar a despesa: ' + (err.message || err));
    }
  }
  ...
  App.correctRegisteredExpense = (id) => setExpenseStatusFixed(id, 'CorreÃ§Ã£o Solicitada');

  // Garante que a aba AprovaÃ§Ã£o de Despesas continue exibindo sÃ³ pendentes.
  const oldRenderExpenses = UI.renderExpenses?.bind(UI);
      ...
      [...body.querySelectorAll('tr')].forEach((tr, idx) => {
        ...
        if (approvalMode && isPending(exp.status)) {
          actionTd.innerHTML = `
            ...
            <button class="btn btn-secondary btn-sm" onclick="event.stopPropagation(); App.correctRegisteredExpense('${String(exp.id).replace(/'/g,"\\'")}')">CorreÃ§Ã£o</button>`;
        }
      });
    };
  }
...
/* CorreÃ§Ãµes gerais 30/06 - permissÃµes, notificaÃ§Ãµes, correÃ§Ãµes pendentes e estabilidade */
(function(){
  'use strict';
  ...
  function ensurePermissionOptions(){
    ...
    add('ConfirmaÃ§Ã£o de Troca', 'ConfirmaÃ§Ã£o de Troca');
    add('MovimentaÃ§Ã£o', 'MovimentaÃ§Ã£o de Equipamentos');
  }

  function applyQuickPermissions(){
    const map = [
      ['#dashboard .quick-actions-grid button[onclick*="prospeccao"]', ['ProspecÃ§Ã£o','Leads']],
      ['#dashboard .quick-actions-grid button[onclick*="clientes"]', ['Clientes']],
      ['#dashboard .quick-actions-grid button[onclick*="chamados"]', ['Chamados','Chamados MecÃ¢nicos']],
      ...
      ['.quick-actions-grid button[onclick*="movimentacao"]', ['MovimentaÃ§Ã£o','Equipamentos']]
    ];
  ...
  if (window.App && !App.correctExpenseAndResubmit) {
    App.correctExpenseAndResubmit = async function(id){
      if (!id) return alert('NÃ£o foi possÃ­vel identificar a despesa. Abra os detalhes e tente novamente.');
      try {
        ...
        const obs = prompt('ObservaÃ§Ã£o da correÃ§Ã£o:', current.observation || '') || '';
        await api(`/api/despesas-reembolsos/${encodeURIComponent(id)}/correct`, {
          method: 'PUT',
          body: JSON.stringify({ value: novoValor, observation: obs })
        });
        alert('Despesa corrigida e reenviada para aprovaÃ§Ã£o.');
  ...
  async function setupNotifications(){
    ...
    if (!('serviceWorker' in navigator) || !('PushManager' in window) || !('Notification' in window)) {
      if (container) container.innerHTML = '<div class="alert alert-warning">NotificaÃ§Ãµes nÃ£o suportadas neste navegador.</div>';
      return;
    }
    try {
    ...
    } catch (err) { console.warn('Push nÃ£o habilitado:', err.message); }
  }
    ...
    try {
      ...
      if (window.location.hash === '#notificacoes') {
        ...
        if (container) {
          ...
          if (!list || !list.length) {
            container.innerHTML = '<div style="text-align:center; padding:20px; color:var(--text-muted);">Nenhuma notificaÃ§Ã£o encontrada.</div>';
          } else {
  ...
  if (window.App) {
    ...
    App.setupPushNotificationsManual = async function(options) {
      ...
      if (!window.isSecureContext) return fail('NotificaÃ§Ãµes Push exigem HTTPS. No Render/domÃ­nio HTTPS deve funcionar normalmente.');
      if (!('serviceWorker' in navigator)) return fail('Este navegador nÃ£o suporta Service Worker. Use Chrome ou Edge atualizado.');
      if (!('Notification' in window)) return fail('Este navegador nÃ£o suporta a API de NotificaÃ§Ãµes. Use Chrome ou Edge atualizado.');
      if (!('PushManager' in window)) return fail('Este navegador nÃ£o suporta PushManager/Web Push. Use Chrome ou Edge atualizado.');
      try {
        const reg = await navigator.serviceWorker.getRegistration() || await navigator.serviceWorker.register('/sw.js');
        const keyResp = await api('/api/push/vapid-public-key').catch(()=>({publicKey:''}));
        if (!keyResp || !keyResp.publicKey) return fail('Chave pÃºblica VAPID nÃ£o encontrada no servidor. Configure VAPID_PUBLIC_KEY e VAPID_PRIVATE_KEY no Render.');
        ...
        if (permission !== 'granted') return fail('PermissÃ£o de notificaÃ§Ã£o negada. Ative as notificaÃ§Ãµes deste site nas configuraÃ§Ãµes do navegador e clique novamente em Receber Push no Celular.');
        ...
        if (!silent) alert('NotificaÃ§Ãµes Push ativadas com sucesso neste dispositivo!');
        return true;
      } catch (err) {
        console.error(err);
        return fail('Erro ao ativar notificaÃ§Ãµes: ' + (err.message || err));
      }
    };
  }
...
/* CorreÃ§Ãµes finais 30/06: correÃ§Ã£o completa de despesa + permissÃ£o do gestor de equipamentos */
(function(){
  'use strict';
  const moneyNum = (v) => {
    if (v === null || v === undefined || v === '') return 0;
  ...
  function canConfirmMovement(){
    ...
    // Regra obrigatÃ³ria: vendedor e supervisor sÃ³ visualizam o dossiÃª.
    // Mesmo que tenham alguma permissÃ£o genÃ©rica marcada, nÃ£o podem aprovar/reprovar.
    ...
    return hasPerm('ConfirmaÃ§Ã£o de MovimentaÃ§Ã£o') || hasPerm('ConfirmaÃ§Ã£o de Troca') || hasPerm('AvaliaÃ§Ã£o de MovimentaÃ§Ã£o');
  }
  ...
  function ensureCurrentMediaBox(kind, url){
    ...
    box.innerHTML = `<div style="margin-bottom:6px;font-weight:600;">Arquivo atual mantido. Envie outro somente se precisar substituir.</div>
      <img src="${src.replace(/"/g,'&quot;')}" style="max-height:90px;max-width:160px;border-radius:4px;border:1px solid var(--border-color);cursor:pointer;object-fit:cover;"
        ...
        onerror="this.style.display='none'; this.insertAdjacentHTML('afterend','<div style=&quot;color:#f59e0b;&quot;>Imagem antiga nÃ£o encontrada. Envie novamente para substituir.</div>');">`;
  }
  ...
  async function api(path, options={}){
    ...
    if (!res.ok) throw new Error(data.error || data.message || 'Erro na requisiÃ§Ã£o');
    return data;
  }
  ...
  function installCorrectionSubmitHandler(){
    ...
    form.addEventListener('submit', async function(e){
      ...
      try {
        ...
        if (window.App?.showToast) App.showToast('Despesa corrigida e reenviada para aprovaÃ§Ã£o.'); else alert('Despesa corrigida e reenviada para aprovaÃ§Ã£o.');
  ...
  if (window.App) {
    App.correctExpenseAndResubmit = async function(id){
      if (!id) return alert('NÃ£o foi possÃ­vel identificar a despesa.');
      try {
        const current = await api(`/api/despesas-reembolsos/${encodeURIComponent(id)}`);
        if (String(current.status || '') !== 'CorreÃ§Ã£o Solicitada') return alert('Esta despesa nÃ£o estÃ¡ aguardando correÃ§Ã£o.');
        if (String(current.userId) !== String(currentUser().id) && !hasPerm('Administrador')) return alert('VocÃª sÃ³ pode corrigir despesas lanÃ§adas por vocÃª.');
        window.location.hash = '#despesas';
        setTimeout(() => {
          const form = document.getElementById('expense-form');
          const container = document.getElementById('expense-form-container');
          if (!form || !container) return alert('FormulÃ¡rio de despesa nÃ£o carregou. Tente novamente.');
          ...
          const btn = form.querySelector('button[type="submit"]'); if (btn) btn.textContent = 'Salvar CorreÃ§Ã£o e Reenviar';
        ...
        alert('Erro ao abrir despesa para correÃ§Ã£o: ' + (err.message || err.error || err));
      }
    };
      ...
      App.showMovementDetails = async function(id){
        ...
        if (panel) {
          if (canConfirmMovement() && mov && mov.status === 'Pendente') {
            ...
            if (subBlock) {
              const needsManagerEquipment = ['Troca', 'AdiÃ§Ã£o'].includes(mov.tipo_solicitacao);
              ...
              if (titleEl) titleEl.textContent = mov.tipo_solicitacao === 'AdiÃ§Ã£o' ? 'âœ… EQUIPAMENTO CONFIRMADO NA ADIÃ‡ÃƒO' : 'ðŸ”„ EQUIPAMENTO DE SUBSTITUIÃ‡ÃƒO';
              if (helpEl) helpEl.textContent = mov.tipo_solicitacao === 'AdiÃ§Ã£o' ? 'Preencha o patrimÃ´nio, modelo e voltagem confirmados para instalar no cliente.' : 'Preencha os dados do equipamento que serÃ¡ enviado para substituiÃ§Ã£o. NÃ£o use listas prÃ©-definidas â€” preencha manualmente.';
            }
          } else {
        ...
        // AdiÃ§Ã£o specs block
        const specsGrid = document.querySelector('#dossie-eq-padrao-container > div');
        if (specsGrid) {
          specsGrid.style.display = (isSeller && mov && mov.tipo_solicitacao === 'AdiÃ§Ã£o') ? 'none' : 'grid';
        }
        
        // Timeline link
        const timelineLink = document.getElementById('btn-show-timeline-dossie');
        if (timelineLink) {
          timelineLink.style.display = (isSeller && mov && mov.tipo_solicitacao === 'AdiÃ§Ã£o') ? 'none' : '';
        }
      };
    }
  }
  ...
  if (oldRenderExpenses) {
    UI.renderExpenses = function(expenses){
      ...
      document.querySelectorAll('#expenses-table-body tr').forEach((tr, idx) => {
        ...
        if (!cell) return;
        if (String(exp.status || '') === 'CorreÃ§Ã£o Solicitada' && String(exp.userId) === String(user.id)) {
...
/* ============================================================
   CORREÃ‡Ã•ES FINAIS - 30/06
   1. ExclusÃ£o correta de clientes (via API + local store)
   2. Filtro global de vendedor (dropdown populado e funcional)
   3. Datas formatadas no fuso de BrasÃ­lia em toda a UI
   4. NotificaÃ§Ãµes: banner para navegadores sem suporte
   ============================================================ */
(function () {
  'use strict';
  ...
  /* 1. EXCLUSÃƒO CORRETA DE CLIENTES */
  function injectDeleteClient() {
    ...
    App.deleteClient = async function (id) {
      ...
      // ForÃ§a sincronizaÃ§Ã£o do banco para nÃ£o trazer o cliente de volta
  ...
  // Injeta botÃ£o Excluir nas linhas de clientes (somente admin)
...
(function(){
  ...
  const scoreText = (c) => {
    ...
    return `${score !== '' ? 'Score ' + esc(score) : 'Score -'}${cls ? ' â€¢ ' + esc(cls) : ''}`;
  };
  ...
  function renderPhotoCard(label, url){
    ...
    if (!finalUrl) return `<div class="photo-card"><b>${esc(label)}</b><div class="photo-empty">Imagem nÃ£o enviada</div></div>`;
    return `<div class="photo-card"><b>${esc(label)}</b><img src="${esc(finalUrl)}" alt="${esc(label)}" onclick="App.showFacadeImage && App.showFacadeImage('${esc(finalUrl).replace(/'/g,'\\&#39;')}')" onerror="this.replaceWith(Object.assign(document.createElement('div'),{className:'photo-empty',textContent:'Imagem indisponÃ­vel'}))"><div style="font-size:.65rem;color:var(--text-muted);overflow-wrap:anywhere;margin-top:4px;">${esc(finalUrl)}</div></div>`;
  }
  ...
  function patchUI(){
    ...
    UI.renderClients = function(clients){
      ...
      body.innerHTML = list.map(c => {
        ...
        return `<tr class="cliente-compact-row" onclick="App.showClientDetails('${esc(c.id)}')"><td colspan="10"><div class="cliente-compact-card"><div class="cliente-compact-main"><div class="cliente-compact-name">${esc(c.name || c.nomeFantasia || c.companyName || 'Cliente sem nome')}</div><div class="cliente-compact-meta">${esc(sellerName(c.userId))} â€¢ ${esc(fmtDate(c))} â€¢ ${scoreText(c)}</div></div><div class="cliente-compact-actions"><button class="btn btn-primary btn-xs" onclick="event.stopPropagation(); App.showClientDetails('${esc(c.id)}')">Ver Ficha</button>${adminBtn}</div></div></td></tr>`;
    ...
    UI.showClientDetails = function(client){
      ...
      content.innerHTML = `<div class="client-ficha-compact">
        ...
        <div class="client-section"><h4>1. IdentificaÃ§Ã£o Comercial</h4>${field('Nome Fantasia', client.name)}${field('RazÃ£o Social', client.companyName)}${field('CNPJ', client.cnpj)}${field('InscriÃ§Ã£o Estadual', client.ie)}${field('Categoria', client.category)}${field('Telefone', client.phone)}${field('E-mail', client.email)}${field('Vendedor', sellerName(client.userId))}${field('Unidade', UI.getUnitName ? UI.getUnitName(client.unitId) : client.unitId)}</div>
        <div class="client-section"><h4>2. LogÃ­stica e LocalizaÃ§Ã£o</h4>${field('Cidade', client.city)}${field('UF', client.state)}${field('CEP', client.cep)}${field('Rua', client.street)}${field('NÃºmero', client.number)}${field('Bairro', client.neighborhood)}${field('EndereÃ§o', client.addressFull)}${field('LocalizaÃ§Ã£o', client.locationType)}${field('PavimentaÃ§Ã£o', client.pavementType)}${field('HorÃ¡rio Receb.', client.deliverySchedule)}${field('1Âº Pedido', client.firstOrderPayment)}${field('Motivo', client.firstOrderReason)}${field('Recompra', client.repurchasePayment)}</div>
        <div class="client-section"><h4>3. Mercado</h4>${field('Amaretto prÃ³ximo', client.nearbyAmaretto)}${field('ConcorrÃªncia', client.nearbyCompetitor)}${field('JÃ¡ trabalha com sorvete', client.iceCreamExperience)}${field('Duas marcas', client.dualBrandPreference)}</div>
        <div class="client-section"><h4>4. Equipamentos e Financeiro</h4>${field('Qtd Equip.', client.equipmentQty)}${field('Equip. solicitado', client.requestedEqType)}${field('PadrÃ£o envio', client.sendableEqType)}${field('Produtos', products)}${field('Valor 1Âª compra', money(client.firstOrderValue))}${field('MÃ©dia mensal', money(client.predictedAverage))}${field('BonificaÃ§Ã£o', client.hasBonus)}${field('Valor bonificaÃ§Ã£o', client.bonusValue ? money(client.bonusValue) : '')}</div>
        <div class="client-section"><h4>5. AnÃ¡lise do Vendedor</h4><div class="client-value" style="font-style:italic;">${esc(client.sellerAnalysis || 'Nenhuma anÃ¡lise inserida.')}</div></div>
  ...
  function patchApp(){
    ...
    // Ao abrir cadastro novo, limpa apenas previews e campos de arquivo, sem mexer em dados apÃ³s erro.
  ...
  function fixPaymentOptions(){
    ...
    sel.innerHTML = '<option value="" selected disabled>Selecione...</option><option value="Ã€ vista">Ã€ vista</option><option value="Boleto">Boleto</option><option value="Metade Ã  vista, metade no boleto">Metade Ã  vista, metade no boleto</option>';
    if (['Ã€ vista','Boleto','Metade Ã  vista, metade no boleto'].includes(current)) sel.value = current;
 ...
 * CorreÃ§Ã£o 01/07: relatÃ³rios Excel completos, cabeÃ§alhos em portuguÃªs e links das mÃ­dias.
 */
(function() {
  'use strict';
  ...
  const REPORT_TITLES = {
    ...
    aprovacao: 'CLIENTES EXTERNOS / APROVAÃ‡ÃƒO',
    prospeccao: 'PROSPECÃ‡ÃƒO',
    equipamentos: 'EQUIPAMENTOS',
    movimentacao: 'MOVIMENTAÃ‡ÃƒO DE EQUIPAMENTOS',
    chamados: 'CHAMADOS MECÃ‚NICOS',
    despesas: 'DESPESAS DE VIAGEM',
    'solicitacao-despesas': 'SOLICITAÃ‡Ã•ES DE SALDO',
    usuarios: 'USUÃRIOS',
    'simulador-troca': 'SIMULADOR DE TROCA',
    notificacoes: 'NOTIFICAÃ‡Ã•ES'
  };
  ...
  const FiltersManager = {
    ...
    ensureFilterPanel(moduleKey) {
      ...
      if (!bar) {
        ...
        bar.innerHTML = `<div class="filter-fields-row" style="display:flex;flex-wrap:wrap;gap:12px;width:100%;align-items:flex-start;">${this.buildFilterControls(moduleKey)}</div><div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;justify-content:flex-end;width:100%;border-top:1px dashed var(--border-color);padding-top:10px;margin-top:6px;"><button type="button" class="btn btn-secondary btn-clear-filters" style="height:32px;padding:0 12px;font-size:.78rem;">âœ• Limpar Filtros</button><button type="button" class="btn btn-success btn-export-excel" style="height:32px;padding:0 12px;font-size:.78rem;background-color:#10b981;border:1px solid #059669;color:#fff;">ðŸ“¥ Exportar Excel</button><button type="button" class="btn btn-secondary btn-export-all" style="height:32px;padding:0 12px;font-size:.78rem;">ðŸ—‚ï¸ Exportar Tudo</button></div>`;
    ...
    buildFilterControls(moduleKey) {
      const labels = { empresa:'Empresa', unitId:'Unidade', city:'Cidade', category:'Categoria', status:'Status', vendedor:'Vendedor', supervisor:'Supervisor', type:'Tipo', model:'Modelo', serial:'PatrimÃ´nio', situation:'SituaÃ§Ã£o', plate:'Placa', number:'NÃºmero OS', responsible:'ResponsÃ¡vel', profile:'Perfil', client:'Cliente', priority:'Prioridade' };
    ...
    getUniqueValues(data, field) {
      ...
      (Array.isArray(data) ? data : []).forEach(item => {
        ...
        if (value && !['null','undefined','â€”','-'].includes(value.toLowerCase())) set.add(value);
      });
    ...
    getFilterValue(item, field) {
      ...
      if (field === 'status') return item.read !== undefined ? (item.read ? 'Lida' : 'NÃ£o lida') : pick(item, ['status']);
    ...
    exportExcel(moduleKey, useFiltered) {
      if (!window.XLSX) return alert('Biblioteca Excel (SheetJS) nÃ£o carregada. Aguarde ou recarregue a pÃ¡gina.');
      ...
      const meta = [
        [`RELATÃ“RIO: ${REPORT_TITLES[moduleKey] || moduleKey.toUpperCase()}`],
        ['Data da ExportaÃ§Ã£o:', new Date().toLocaleDateString('pt-BR') + ' ' + new Date().toLocaleTimeString('pt-BR')],
        ['UsuÃ¡rio ResponsÃ¡vel:', loggedUser ? `${loggedUser.name || ''} (${loggedUser.username || loggedUser.id || ''})` : 'Sistema'],
    ...
    getCollectedFiles() {
      ...
      const add = (url, type, item, module) => {
        ...
        files.push({
          ...
          type,
          client: pick(item, ['name','client','cliente_nome','cliente','cliente_nome_fantasia']) || 'â€”',
      ...
      getStoreList('getExpenses').forEach(e => { add(e.foto_odometro || e.photoOdometro, 'Imagem do OdÃ´metro', e, 'Despesas'); add(e.foto_comprovante || e.photo || e.photoComprovante, 'Imagem do Comprovante', e, 'Despesas'); });
      getStoreList('getMovements').forEach(m => ['foto_equipamento_url','foto_antes_url','foto_depois_url','video_url','fotoAntigo','fotoNovo','fotoRecolha','fotoAntes','fotoDepois'].forEach(k => add(m[k], labelMedia(k), m, 'MovimentaÃ§Ãµes')));
    ...
    exportFilesExcel() {
      if (!window.XLSX) return alert('Biblioteca Excel nÃ£o carregada.');
      const rows = this.getCollectedFiles().map(f => ({ 'Nome do Arquivo': f.name, 'Tipo de Arquivo': f.type, 'Cliente Relacionado': f.client, 'Vendedor': f.vendedor, 'Unidade': f.unidade, 'Data de Envio': f.date, 'MÃ³dulo de Origem': f.module, 'Registro Relacionado': f.relatedId, 'Link de Download': f.url }));
  ...
  function expenseRow(e) {
    return {
      ...
      'Tipo de OperaÃ§Ã£o': value(pick(e, ['operacao','operation','tipo_operacao'])),
      'VeÃ­culo': value(pick(e, ['veiculo','vehicle'])),
      'Quilometragem (KM)': value(pick(e, ['km','quilometragem'])),
      'Valor (R$)': money(pick(e, ['value','valor','amount'])),
      'ObservaÃ§Ã£o': value(pick(e, ['observation','observacao','description','descreva'])),
      'Status': value(pick(e, ['status'])),
      'Empresa': value(pick(e, ['empresa_id','company_id','empresa'])),
      'UsuÃ¡rio ResponsÃ¡vel': value(userName(pick(e, ['userId','user_id','usuario_id']))),
      'Link da Imagem do OdÃ´metro': mediaUrl(pick(e, ['foto_odometro','photoOdometro','odometerPhoto'])),
      'Link da Imagem do Comprovante': mediaUrl(pick(e, ['foto_comprovante','photo','photoComprovante','receiptPhoto'])),
      'Outras MÃ­dias': mediaList(e, ['media','midias','attachments','anexos'])
    };
  }
  ...
  function ticketRow(t) {
    return {
      ...
      'MecÃ¢nico ResponsÃ¡vel': value(pick(t, ['mechanic','mecanico'])),
      'Unidade': value(unitName(pick(t, ['unitId','unit_id']))),
      'Vendedor ResponsÃ¡vel': value(pick(t, ['vendedor','seller','seller_name']) || userName(pick(t, ['userId','user_id']))),
      'Tipo de Equipamento': value(pick(t, ['equipmentType','tipo_equipamento'])),
      'PatrimÃ´nio / Serial': value(pick(t, ['equipmentSerial','patrimonio','serial'])),
      'Cliente': value(pick(t, ['client','cliente'])),
      'Nome Fantasia': value(pick(t, ['fantasyName','nomeFantasia'])),
      'Cidade': value(pick(t, ['city','cidade'])),
      'EndereÃ§o': value(pick(t, ['address','endereco'])),
      'DescriÃ§Ã£o Simplificada da Falha': value(pick(t, ['title','falha','falha_relatada'])),
      'Prioridade': value(pick(t, ['priority','prioridade'])),
      'PeÃ§as Utilizadas': listText(pick(t, ['parts','pecas'])),
      'ServiÃ§os Executados': listText(pick(t, ['services','servicos'])),
      'DescriÃ§Ã£o do Problema': value(pick(t, ['faultDescription','descricao_falha'])),
      'SoluÃ§Ã£o Aplicada': value(pick(t, ['solutionDescription','solucao'])),
      'Estado PÃ³s Atendimento': value(pick(t, ['eqStatusAfter','estado_pos'])),
      'Carga de GÃ¡s (g)': value(pick(t, ['gasCharge','carga_gas'])),
      'ObservaÃ§Ãµes': value(pick(t, ['additionalNotes','observations','observacoes'])),
      'Link da Foto do Defeito': mediaUrl(pick(t, ['defectPhoto'])),
      'Link do VÃ­deo do Defeito': mediaUrl(pick(t, ['defectVideo'])),
      ...
      'Link do VÃ­deo do Atendimento': mediaUrl(pick(t, ['videoAtendimento'])),
      'Status': value(pick(t, ['status'])),
      'Hora de ConclusÃ£o': value(pick(t, ['endTime','hora_conclusao']))
    };
  }
  ...
  function movementRow(m) {
    return {
      ...
      'Tipo de SolicitaÃ§Ã£o': value(pick(m, ['tipo_solicitacao','type','tipo'])),
      ...
      'CÃ³digo do Cliente': value(pick(m, ['cliente_codigo','clientCode'])),
      'Nome Fantasia': value(pick(m, ['cliente_nome','clientName','name'])),
      'Cidade': value(pick(m, ['cliente_cidade','clientCity','city'])),
      'EndereÃ§o Comercial': value(pick(m, ['cliente_endereco','clientAddress','address'])),
      'Vendedor ResponsÃ¡vel': value(pick(m, ['cliente_vendedor','sellerName'])),
      'PatrimÃ´nio': value(pick(m, ['patrimonio','serial'])),
      'Modelo': value(pick(m, ['modelo','model'])),
      'Voltagem': value(pick(m, ['voltagem','voltage'])),
      'PatrimÃ´nio Novo': value(pick(m, ['patrimonio_novo','newPatrimonio'])),
      ...
      'Link do VÃ­deo': mediaUrl(pick(m, ['video_url','videoTroca']))
    };
  }
  ...
  function clientRow(c) {
    return {
      ...
      'Vendedor ResponsÃ¡vel': value(pick(c, ['vendedor','seller_name']) || userName(pick(c, ['userId','user_id','seller_id']))),
      'Nome do ComÃ©rcio': value(pick(c, ['name','nomeFantasia','tradeName'])),
      'RazÃ£o Social': value(pick(c, ['companyName','razaoSocial','razao_social'])),
      'CNPJ': value(pick(c, ['cnpj'])),
      'InscriÃ§Ã£o Estadual': value(pick(c, ['ie','inscricaoEstadual','inscricao_estadual'])),
      ...
      'NÃºmero': value(pick(c, ['number','numero'])),
      'Bairro': value(pick(c, ['neighborhood','bairro'])),
      'EndereÃ§o Completo': value(pick(c, ['addressFull','enderecoCompleto','address'])),
      'LocalizaÃ§Ã£o do ComÃ©rcio': value(pick(c, ['locationType','localizacaoComercio'])),
      'PavimentaÃ§Ã£o da Rua': value(pick(c, ['pavementType','pavimentacao'])),
      'HorÃ¡rio de Recebimento': value(pick(c, ['deliverySchedule','horarioRecebimento'])),
      'Unidade Vinculada': value(unitName(pick(c, ['unitId','unit_id']))),
      'Ponto Amaretto PrÃ³ximo': value(pick(c, ['nearbyAmaretto','amarettoProximo'])),
      'ConcorrÃªncia PrÃ³xima': value(pick(c, ['nearbyCompetitor','concorrenciaProxima'])),
      'Trabalha com Sorvete/PicolÃ©': value(pick(c, ['iceCreamExperience','trabalhaSorvete'])),
      'TrabalharÃ¡ com Duas Marcas': value(pick(c, ['dualBrandPreference','duasMarcas'])),
      ...
      'Produtos que IrÃ¡ Trabalhar': listText(pick(c, ['products','produtos'])),
      'MÃ©dia Prevista Mensal': money(pick(c, ['predictedAverage','mediaPrevista'])),
      ...
      'BonificaÃ§Ã£o': value(pick(c, ['hasBonus','bonificacao'])),
      'Roteiro Indicado': value(pick(c, ['route','roteiro','sellerRoute'])),
      'AnÃ¡lise do Vendedor': value(pick(c, ['sellerAnalysis','analiseVendedor'])),
  ...
  function value(v) { return (v === undefined || v === null || v === '') ? 'â€”' : v; }
  function money(v) {
  ...
  function formatTime(v) { if (!v) return 'â€”'; if (/^\d{2}:\d{2}/.test(String(v))) return String(v).slice(0,5); const d = new Date(v); return isNaN(d.getTime()) ? 'â€”' : d.toLocaleTimeString('pt-BR', { hour:'2-digit', minute:'2-digit' }); }
  function listText(v) { if (!v) return 'â€”'; if (Array.isArray(v)) return v.join(' | '); try { const a = JSON.parse(v); if (Array.isArray(a)) return a.join(' | '); } catch(_) {} return String(v); }
  function mediaList(obj, keys) { const arr = []; keys.forEach(k => { const v = obj && obj[k]; if (Array.isArray(v)) v.forEach(x => arr.push(mediaUrl(x))); else if (v) arr.push(mediaUrl(v)); }); return arr.filter(x => x && x !== 'â€”').join(' | ') || 'â€”'; }
  function mediaUrl(url) { let raw = Array.isArray(url) ? url[0] : url; if (!raw) return 'â€”'; if (typeof raw === 'object') raw = raw.url || raw.path || raw.src || ''; raw = String(raw || '').trim(); if (!raw || ['null','undefined','â€”','-'].includes(raw.toLowerCase()) || raw.startsWith('data:')) return raw.startsWith('data:') ? 'MÃ­dia salva em base64/local; reenviar para gerar link pÃºblico.' : 'â€”'; if (window.TempPhotosCache && window.TempPhotosCache[raw]) raw = window.TempPhotosCache[raw]; if (raw.startsWith('http://') || raw.startsWith('https://')) return raw; if (raw.startsWith('/')) return window.location.origin + raw; return window.location.origin + '/' + raw.replace(/^\/+/, ''); }
  function labelMedia(k) { return ({ photoFachada:'Foto da Fachada', photoInterna01:'Foto Interna 01', photoInterna02:'Foto Interna 02', photoInterna03:'Foto Interna 03', photoRua01:'Foto Externa Rua 01', photoRua02:'Foto Externa Rua 02', photoCnpj:'Foto do CNPJ', foto_odometro:'Imagem do OdÃ´metro', foto_comprovante:'Imagem do Comprovante', foto_antes_url:'Foto Antes', foto_depois_url:'Foto Depois', foto_equipamento_url:'Foto do Equipamento', video_url:'VÃ­deo', defectPhoto:'Foto do Defeito', defectVideo:'VÃ­deo do Defeito', fotoAntes:'Foto Antes', fotoDepois:'Foto Depois', fotoPlaqueta:'Foto da Plaqueta', videoAtendimento:'VÃ­deo do Atendimento' }[k] || k); }
...
/* CorreÃ§Ã£o adicional 01/07 - PDFs completos via jsPDF, download direto e sem aba about:blank */
(function(){
  'use strict';
  ...
  function val(v){
    if (v === undefined || v === null || v === '' || v === 'null' || v === 'undefined') return 'â€”';
    return String(v);
  }
  ...
  async function imageToDataUrl(url){
    ...
    try {
      ...
      console.warn('NÃ£o foi possÃ­vel carregar imagem no PDF:', src, err);
      return '';
    }
  }
  ...
  async function addImageBox(doc, x, y, w, h, label, url){
    ...
    if (data) {
      try {
        ...
        doc.setFont('helvetica','normal');
        doc.setFontSize(8);
        doc.text('Imagem anexada, mas nÃ£o foi possÃ­vel inserir no PDF.', x+3, y+18);
      }
    } else {
      doc.setFont('helvetica','normal');
      doc.setFontSize(8);
      doc.setTextColor(120,120,120);
      doc.text('Imagem nÃ£o enviada ou indisponÃ­vel.', x+3, y+18);
      doc.setTextColor(0,0,0);
    }
    const link = mediaUrl(url);
  ...
  function footer(doc){
    ...
    for (let i=1;i<=pages;i++){
      ...
      doc.text(`Controle de Campo â€¢ Gerado em ${new Date().toLocaleString('pt-BR')} â€¢ PÃ¡gina ${i} de ${pages}`, 105, 287, {align:'center'});
      doc.setTextColor(0,0,0);
    }
  }
  ...
  async function generateExpensePdf(id){
    try {
      ...
      if (!exp) return alert('Despesa nÃ£o encontrada para gerar PDF.');
      ...
      doc.setFont('helvetica','normal');
      doc.setFontSize(9);
      doc.text(`EmissÃ£o: ${new Date().toLocaleString('pt-BR')}`, 120, y);
      y += 8;

      fieldBox(doc, 14, y, 'Data / Hora', [exp.date || exp.data || exp.created_at || exp.createdAt, exp.time || exp.hora].filter(Boolean).join(' Ã s '), 80);
      ...
      fieldBox(doc, 108, y, 'Tipo de OperaÃ§Ã£o', exp.operacao || exp.operation || exp.tipo_operacao, 80);
      y += 20;
      ...
      if ((String(exp.finalidade || '').toLowerCase() === 'abastecimento') || exp.veiculo || exp.km) {
        y = section(doc, y, '2. Dados do Abastecimento / VeÃ­culo', title);
        fieldBox(doc, 14, y, 'VeÃ­culo', exp.veiculo || exp.vehicle, 80);
        fieldBox(doc, 108, y, 'Quilometragem (KM)', exp.km || exp.quilometragem, 80);
        y += 20;
      }

      y = section(doc, y, '3. ObservaÃ§Ãµes e AprovaÃ§Ã£o', title);
      y = textBlock(doc, y, 'ObservaÃ§Ã£o', exp.observation || exp.observacao || exp.description || exp.descreva, title);
      y = textBlock(doc, y, 'HistÃ³rico / Parecer', exp.approval_note || exp.motivo || exp.justificativa || `Status atual: ${val(exp.status)}`, title);

      y = section(doc, y, '4. Anexos / Comprovantes FotogrÃ¡ficos', title);
      ...
      await addImageBox(doc, 110, y, 88, 82, 'OdÃ´metro / KM', exp.foto_odometro || exp.photoOdometro || exp.odometerPhoto);
      y += 88;
  ...
  function ticketFromFormOrStore(id){
    ...
    if (outraPecaInput && outraPecaInput.value.trim() && document.querySelector('#modal-ficha-tecnica .btn-part-toggle[data-part="Outra PeÃ§a"].active')) parts.push('Outra: ' + outraPecaInput.value.trim());
    ...
    if (outroServicoInput && outroServicoInput.value.trim() && document.querySelector('#modal-ficha-tecnica .btn-part-toggle[data-service="Outro ServiÃ§o"].active')) services.push('Outro: ' + outroServicoInput.value.trim());
  ...
  async function generateTicketPdfById(id){
    try {
      ...
      if (!ticket || !ticket.id) return alert('Chamado nÃ£o encontrado.');

      const title = 'FICHA TÃ‰CNICA DE MANUTENÃ‡ÃƒO - ORDEM DE SERVIÃ‡O';
      const doc = setupDoc(`Ficha TÃ©cnica ${ticket.id}`);
      pageBorder(doc, title);
      ...
      y = section(doc, y, '1. IdentificaÃ§Ã£o do Atendimento', title);
      fieldBox(doc, 14, y, 'MecÃ¢nico ResponsÃ¡vel', ticket.mechanic, 80);
      fieldBox(doc, 108, y, 'Data de RealizaÃ§Ã£o', ticket.date, 80);
      y += 20;
      fieldBox(doc, 14, y, 'Hora de InÃ­cio / ConclusÃ£o', `${val(ticket.startTime)} / ${val(ticket.endTime)}`, 80);
      fieldBox(doc, 108, y, 'Unidade Vinculada', ticket.unit || unitName(ticket.unitId), 80);
      y += 22;

      y = section(doc, y, '2. IdentificaÃ§Ã£o do Equipamento', title);
      fieldBox(doc, 14, y, 'Tipo de Equipamento', ticket.equipmentType, 54);
      fieldBox(doc, 75, y, 'NÂº PatrimÃ´nio / Serial', ticket.equipmentSerial, 54);
      fieldBox(doc, 136, y, 'Cliente Vinculado', ticket.client, 54);
      y += 20;
      fieldBox(doc, 14, y, 'CÃ³digo do Cliente', ticket.clientCode || ticket.cliente_codigo || '-', 54);
      ...
      fieldBox(doc, 75, y, 'Estado PÃ³s Atendimento', ticket.eqStatusAfter, 54);
      y += 20;
      fieldBox(doc, 14, y, 'Cidade', ticket.city || '-', 54);
      fieldBox(doc, 75, y, 'EndereÃ§o', ticket.address || '-', 116);
      y += 20;
      y = textBlock(doc, y, 'DescriÃ§Ã£o Simplificada da Falha', ticket.title, title);

      y = section(doc, y, '3. PeÃ§as Utilizadas', title);
      ...
      y = section(doc, y, '4. ServiÃ§os Executados', title);
      ...
      y = section(doc, y, '5. Laudo e DiagnÃ³stico TÃ©cnico', title);
      y = textBlock(doc, y, 'DescriÃ§Ã£o Detalhada do Problema Encontrado', ticket.faultDescription, title);
      y = textBlock(doc, y, 'SoluÃ§Ã£o Aplicada / Laudo TÃ©cnico', ticket.solutionDescription, title);
      fieldBox(doc, 14, y, 'Carga de GÃ¡s (gramas)', ticket.gasCharge ? `${ticket.gasCharge}g` : 'â€”', 80);
      fieldBox(doc, 108, y, 'ObservaÃ§Ãµes Adicionais', ticket.additionalNotes, 80);
      y += 23;

      y = section(doc, y, '6. Fotos e VÃ­deo da Visita', title);
      ...
      if (v1 || v2) {
        ...
        doc.setFont('helvetica','bold');
        doc.setFontSize(8);
        doc.text('Links de vÃ­deos:', 14, y);
        y += 5;
        doc.setFont('helvetica','normal');
        doc.setFontSize(7);
        doc.setTextColor(37,99,235);
        if (v1) { doc.text(doc.splitTextToSize('VÃ­deo do defeito: ' + v1, 180), 14, y); y += 8; }
        if (v2) { doc.text(doc.splitTextToSize('VÃ­deo do atendimento: ' + v2, 180), 14, y); y += 8; }
        doc.setTextColor(0,0,0);
      }
      ...
      doc.setFont('helvetica','bold');
      doc.setFontSize(8);
      doc.text('Assinatura do TÃ©cnico', 55, y+5, {align:'center'});
      doc.text('Assinatura do Cliente / ResponsÃ¡vel', 155, y+5, {align:'center'});
  ...
  function install(){
    ...
    App.generateTicketPdfFromForm = function(){
      ...
      if (!id) return alert('Nenhuma Ordem de ServiÃ§o carregada no formulÃ¡rio.');
...
/* CorreÃ§Ã£o: card ultra compacto e modal bonito para chamados mecÃ¢nicos */
(function(){
  'use strict';
  ...
  function priorityClass(priority) { if (priority === 'Alta') return 'badge-danger'; if (priority === 'MÃ©dia') return 'badge-warning'; return 'badge-primary'; }
  function mediaItems(ticket) {
    ...
    if (validUrl(ticket.defectVideo)) items.push({ url: ticket.defectVideo, label: 'VÃ­deo do Defeito', kind: 'video' });
    ...
    if (validUrl(ticket.videoAtendimento)) items.push({ url: ticket.videoAtendimento, label: 'VÃ­deo Atendimento', kind: 'video' });
    return items;
  }
  ...
  function item(label, value, full) {
    ...
    return '<div class=\"cc-detail-item' + (full ? ' cc-detail-wide' : '') + '\"><small>' + esc(label) + '</small><b>' + (has ? esc(value) : 'â€”') + '</b></div>';
  }
  ...
  function installRenderTickets(){
    ...
    UI.renderTickets = function(tickets) {
      ...
      if (!body) return;
      const isStaff = user && ['Administrador','ResponsÃ¡vel Equipamentos','MecÃ¢nico'].includes(user.profile);
      body.innerHTML = list.map(function(ticket){
        const clientName = ticket.fantasyName || ticket.client || 'Cliente nÃ£o informado';
        const city = ticket.city ? ' â€¢ ' + ticket.city : '';
        const equip = [ticket.equipmentSerial, ticket.equipmentType].filter(Boolean).join(' â€¢ ') || 'Equipamento nÃ£o informado';
        const resp = seller(ticket) || ticket.mechanic || '';
        const anexos = mediaItems(ticket).length;
        const anexosText = anexos ? ' â€¢ ' + anexos + ' anexo' + (anexos > 1 ? 's' : '') : '';
        return '<tr class=\"mobile-summary-row cc-ticket-compact-row\" onclick=\"App.showTicketDetails(\'' + esc(ticket.id) + '\')\">' +
          '<td colspan=\"12\">' +
            '<div style=\"display:grid;gap:4px;\">' +
              '<div style=\"display:flex;align-items:center;justify-content:space-between;gap:8px;min-width:0;\"><div class=\"cc-line\" style=\"font-size:.82rem;font-weight:800;\"><span style=\"font-family:monospace;font-weight:900;\">' + esc(ticket.id || '') + '</span> â€¢ ' + esc(clientName) + esc(city) + '</div><span style=\"display:flex;align-items:center;gap:4px;flex-shrink:0;\"><span class=\"badge-status ' + priorityClass(ticket.priority) + '\">' + esc(ticket.priority || 'â€”') + '</span><span class=\"badge-status ' + statusClass(ticket.status) + '\">' + esc(ticket.status || 'â€”') + '</span></span></div>' +
              '<div class=\"cc-line\" style=\"font-size:.72rem;color:var(--text-muted);font-weight:700;\">' + esc(equip) + ' â€¢ ' + esc(ticket.date || 'S/D') + (resp ? ' â€¢ ' + esc(resp) : '') + esc(anexosText) + '</div>' +
  ...
  function installDetails(){
    ...
    App.showTicketDetails = function(id) {
      ...
      if (!ticket) return alert('Chamado nÃ£o encontrado.');
      ...
      const mediaHtml = (function(){
        ...
        return '<section class=\"cc-detail-card\"><h4 class=\"cc-detail-title\">Anexos</h4><div style=\"display:grid;grid-template-columns:repeat(auto-fill,minmax(84px,1fr));gap:8px;\">' + items.map(function(media){
          ...
          const body = media.kind === 'video' ? '<a href=\"' + esc(finalUrl) + '\" target=\"_blank\" style=\"display:flex;align-items:center;justify-content:center;height:58px;color:var(--primary-color);font-weight:800;font-size:.72rem;text-decoration:none;\">VÃ­deo</a>' : '<img src=\"' + esc(finalUrl) + '\" style=\"width:100%;height:58px;object-fit:cover;border-radius:6px;cursor:pointer;\" onclick=\"App.showFacadeImage(\'' + esc(finalUrl) + '\')\" onerror=\"this.parentElement.style.display=\'none\'\">';
      ...
      const html =
        ...
        card('Abertura', item('Vendedor', seller(ticket)) + item('Tipo', ticket.equipmentType) + item('PatrimÃ´nio', ticket.equipmentSerial) + item('Cliente', ticket.client) + item('Fantasia', ticket.fantasyName) + item('Cidade', ticket.city) + item('EndereÃ§o', ticket.address, true) + item('Falha', ticket.title, true) + item('Obs.', ticket.observations, true)) +
        card('Atendimento', item('MecÃ¢nico', ticket.mechanic) + item('InÃ­cio', ticket.startTime) + item('ConclusÃ£o', ticket.endTime) + item('SituaÃ§Ã£o', ticket.eqStatusAfter) + item('PeÃ§as', parts, true) + item('ServiÃ§os', services, true) + item('Problema', ticket.faultDescription, true) + item('SoluÃ§Ã£o', ticket.solutionDescription, true) + item('GÃ¡s (g)', ticket.gasCharge) + item('Obs. finais', ticket.additionalNotes, true)) + mediaHtml;
...
/* CorreÃ§Ã£o: detalhe profissional de chamado em desktop e mobile */
(function(){
  'use strict';
  ...
  function mediaItems(ticket){ var items=[]; if(validUrl(ticket.defectPhoto)) items.push({url:ticket.defectPhoto,label:'Foto do defeito',kind:'image'}); if(validUrl(ticket.defectVideo)) items.push({url:ticket.defectVideo,label:'VÃ­deo do defeito',kind:'video'}); if(validUrl(ticket.fotoAntes)) items.push({url:ticket.fotoAntes,label:'Foto antes',kind:'image'}); if(validUrl(ticket.fotoDepois)) items.push({url:ticket.fotoDepois,label:'Foto depois',kind:'image'}); if(validUrl(ticket.fotoPlaqueta)) items.push({url:ticket.fotoPlaqueta,label:'Plaqueta',kind:'image'}); if(validUrl(ticket.videoAtendimento)) items.push({url:ticket.videoAtendimento,label:'VÃ­deo atendimento',kind:'video'}); return items; }
  function injectStyle(){
  ...
  function install(){
    ...
    App.showTicketDetails=function(id){
      ...
      var subtitle=[ticket.fantasyName||ticket.client,ticket.city,ticket.equipmentSerial].filter(Boolean).join(' â€¢ ');
      ...
      var abertura=card('Abertura do chamado',field('Unidade',unit(ticket))+field('Vendedor',seller(ticket))+field('Tipo',ticket.equipmentType)+field('PatrimÃ´nio',ticket.equipmentSerial)+field('Cliente',ticket.client)+field('Fantasia',ticket.fantasyName)+field('Cidade',ticket.city)+field('EndereÃ§o',ticket.address)+field('Falha relatada',ticket.title,true)+field('ObservaÃ§Ãµes',ticket.observations,true));
      var atendimento=card('Atendimento tÃ©cnico',field('MecÃ¢nico',ticket.mechanic)+field('InÃ­cio',ticket.startTime)+field('ConclusÃ£o',ticket.endTime)+field('SituaÃ§Ã£o pÃ³s',ticket.eqStatusAfter)+field('PeÃ§as utilizadas',parts,true)+field('ServiÃ§os executados',services,true)+field('Problema encontrado',ticket.faultDescription,true)+field('SoluÃ§Ã£o aplicada',ticket.solutionDescription,true)+field('Carga de gÃ¡s',ticket.gasCharge)+field('ObservaÃ§Ãµes finais',ticket.additionalNotes,true));
...
/* CorreÃ§Ã£o: modal de chamados redimensionÃ¡vel no desktop */
(function(){
  'use strict';
 ...
 * Adiciona uma guia isolada na tela GestÃ£o de Clientes, sem alterar funÃ§Ãµes existentes.
 * Os dados importados ficam separados e agora sÃ£o salvos tambÃ©m no banco via app_kv_store.
 */
(function () {
  'use strict';
  ...
  const VISIBLE_FIELDS = [
    { key: 'codigo', label: 'CÃ³digo', required: true, aliases: ['codigo', 'cÃ³digo', 'cod', 'cÃ³d', 'codigo cliente', 'codigo do cliente', 'cÃ³d cliente', 'cod cliente'] },
    ...
    { key: 'email', label: 'Email', required: false, aliases: ['email', 'e-mail', 'mail', 'correio eletrÃ´nico'] },
    { key: 'empresaResponsavel', label: 'Empresa ResponsÃ¡vel', required: false, aliases: ['empresa responsavel', 'empresa responsÃ¡vel', 'empresa', 'distribuidora', 'empresa base'] },
    { key: 'cidade', label: 'Cidade', required: false, aliases: ['cidade', 'municipio', 'municÃ­pio'] },
    { key: 'vendedor', label: 'Vendedor', required: false, aliases: ['vendedor', 'representante', 'consultor', 'responsavel comercial', 'responsÃ¡vel comercial'] },
    { key: 'supervisor', label: 'Supervisor', required: false, aliases: ['supervisor', 'gerente', 'coordenador'] }
  ];

  // Campos que sÃ£o importados e salvos no banco, mas ficam ocultos na tabela principal.
  // Eles serÃ£o usados futuramente sem poluir a tela atual.
  const HIDDEN_FIELDS = [
    { key: 'cpf', label: 'CPF', required: false, hidden: true, aliases: ['cpf', 'cpf/cnpj', 'cnpj/cpf', 'documento', 'cadastro pessoa fisica', 'cadastro pessoa fÃ­sica'] },
    { key: 'logradouro', label: 'Logradouro', required: false, hidden: true, aliases: ['logradouro', 'tipo logradouro', 'tipo de logradouro'] },
    { key: 'endereco', label: 'EndereÃ§o', required: false, hidden: true, aliases: ['endereco', 'endereÃ§o', 'rua', 'avenida', 'av', 'logradouro endereco', 'logradouro endereÃ§o'] },
    { key: 'numero', label: 'NÃºmero', required: false, hidden: true, aliases: ['numero', 'nÃºmero', 'num', 'nÂº', 'nro'] },
    ...
    { key: 'enderecoCompleto', label: 'EndereÃ§o Completo', required: false, hidden: true, calculated: true, aliases: ['endereco completo', 'endereÃ§o completo', 'endereco completo cliente', 'endereÃ§o completo cliente'] }
  ];
  ...
  async function syncRowsFromBackend(force = false) {
    ...
    try {
      ...
      // Se o banco jÃ¡ tem dados, ele vence e todos os aparelhos recebem a mesma lista.
      if (remoteRows.length) {
        ...
        if (window.Store && typeof Store.saveList === 'function') {
          // Atualiza o cache global do Store sem reenviar quando nÃ£o for administrador.
          const storageKey = 'controle_campo_db_global_' + STORE_KEY;
          localStorage.setItem(storageKey, JSON.stringify(remoteRows));
        }
      // Se o banco ainda estÃ¡ vazio, nÃ£o apaga os clientes que jÃ¡ existem no computador.
      // Quando for admin, envia essa lista local para o banco para o celular e outros logins carregarem tambÃ©m.
      ...
      console.warn('NÃ£o foi possÃ­vel sincronizar clientes importados do banco:', err.message || err);
      return false;
    } finally {
  ...
  function getRows() {
    try {
      ...
      if (window.Store && typeof Store.getList === 'function') {
        ...
        // MigraÃ§Ã£o automÃ¡tica da primeira versÃ£o, que salvava somente no navegador.
  ...
  async function saveRows(rows) {
    ...
    // Backup local para nÃ£o perder a importaÃ§Ã£o caso a internet caia no meio.
    ...
    // Garante a gravaÃ§Ã£o no banco antes de fechar o modal e avisar sucesso.
  ...
  function injectStyle() {
    ...
    style.textContent = `
      ...
      /* Ajuste fino do importador no celular: menos poluiÃ§Ã£o visual, sem perder a aÃ§Ã£o de abrir detalhes. */
      @media (max-width: 768px) {
        ...
        /* Na listagem mobile deixa sÃ³ os dados essenciais. O restante abre no card completo ao tocar na linha. */
      ...
      /* CorreÃ§Ã£o final: filtros do importador em 2 colunas no celular, como Clientes Cadastrados. */
      ...
      /* Garante duas colunas de filtros tambÃ©m em celulares comuns de 390/414 px. */
      ...
      /* CorreÃ§Ã£o mobile: o card de detalhes do cliente nÃ£o deve cobrir o cabeÃ§alho fixo. */
      @media (max-width: 900px) {
        #modal-clientes-importador-detail.clientes-importador-modal-overlay {
          top:118px !important;
          right:0 !important;
        ...
        <div class="card" id="clientes-importador-card">
          ...
          <div class="general-filter-bar no-print" style="padding:16px;background:rgba(255,255,255,.02);border-bottom:1px solid var(--border-color);display:flex;flex-direction:column;gap:12px;">
            <div class="clientes-importador-filter-row">
              ...
              ${filterSelect('clientes-importador-empresa', 'Empresa ResponsÃ¡vel')}
            ...
            <div class="clientes-importador-actions">
              <button type="button" class="btn btn-secondary" id="btn-clientes-importador-clear" style="height:32px;padding:0 12px;font-size:.78rem;">âœ• Limpar Filtros</button>
              <button type="button" class="btn btn-success" id="btn-clientes-importador-export-filtered" style="height:32px;padding:0 12px;font-size:.78rem;background-color:#10b981;border:1px solid #059669;color:#fff;">ðŸ“¥ Exportar Excel</button>
              <button type="button" class="btn btn-secondary" id="btn-clientes-importador-export-all" style="height:32px;padding:0 12px;font-size:.78rem;">ðŸ—‚ï¸ Exportar Tudo</button>
            </div>
          </div>
          ...
          <div class="clientes-importador-modal">
            <div class="clientes-importador-modal-header">
              <div>
                ...
                <p class="clientes-importador-modal-subtitle">Envie uma planilha Excel ou CSV, confira o mapeamento das colunas e revise a prÃ©via antes de confirmar. CNPJ pode ficar vazio quando houver CPF.</p>
              </div>
              <button type="button" class="clientes-importador-close" id="btn-clientes-importador-close" aria-label="Fechar">Ã—</button>
            </div>

            <div class="clientes-importador-step">
              <h4>1. Arquivo</h4>
              <input type="file" id="clientes-importador-file-input" accept=".xlsx,.xls,.csv">
              <p style="font-size:.75rem;color:var(--text-muted);margin:8px 0 0;">A primeira linha da planilha deve conter o cabeÃ§alho das colunas.</p>
            </div>
            ...
            <div class="clientes-importador-step" id="clientes-importador-preview-step" style="display:none;">
              <h4>3. PrÃ©via da ImportaÃ§Ã£o</h4>
            ...
            <div class="clientes-importador-modal-footer">
              ...
              <button type="button" class="btn btn-secondary" id="btn-clientes-importador-errors-xlsx" style="display:none;">Baixar RelatÃ³rio de Erros</button>
              <button type="button" class="btn btn-primary" id="btn-clientes-importador-confirm" disabled>Confirmar ImportaÃ§Ã£o</button>
            </div>
          </div>
        </div>
        ...
        <div id="modal-clientes-importador-detail" class="clientes-importador-modal-overlay" role="dialog" aria-modal="true" aria-labelledby="clientes-importador-detail-title">
          <div class="clientes-importador-detail-modal">
            <div class="clientes-importador-detail-head">
              <div>
                <h3 id="clientes-importador-detail-title" class="clientes-importador-detail-title">InformaÃ§Ãµes do Cliente</h3>
              ...
              <button type="button" class="clientes-importador-close" id="btn-clientes-importador-detail-close" aria-label="Fechar">Ã—</button>
            </div>
  ...
  function ensureUi() {
    ...
    // Se a aba/conteÃºdo precisou ser recriado, forÃ§amos o rebinding dos eventos sÃ³ dessa guia.
    if (createdOrRebuilt) {
    ...
    // Renderiza apenas uma vez por montagem da tela para nÃ£o gerar loop de MutationObserver.
  ...
  function bindEvents(panel) {
    ...
    // O card de detalhes fica estÃ¡tico na tela e fecha somente pelo botÃ£o X,
    // para nÃ£o fechar sem querer ao tocar fora no celular.
  ...
  function refreshFilterOptions(preserveKey = null) {
    ...
    // Filtros encadeados: cada lista mostra apenas opÃ§Ãµes compatÃ­veis
    // com os demais filtros jÃ¡ selecionados. Ex.: ao escolher a empresa,
    ...
    // Primeiro limpa seleÃ§Ãµes que nÃ£o existem mais dentro do filtro atual.
    // O filtro que acabou de ser alterado Ã© preservado para nÃ£o voltar para "Todos"
    // quando outro filtro antigo ficar incompatÃ­vel com a nova escolha.
    ...
    // Depois remonta as opÃ§Ãµes jÃ¡ considerando as seleÃ§Ãµes incompatÃ­veis limpas.
    selects.forEach(({ key, el }) => {
      const current = el.value;
  ...
  function renderTable() {
    ...
    tbody.innerHTML = pageRows.map((row, index) => `
      <tr data-detail-index="${index}" title="Clique para abrir as informaÃ§Ãµes completas do cliente">
  ...
  function renderPagination(total, currentPage, totalPages, start, end) {
    ...
    pagination.innerHTML = `
      <div class="clientes-importador-pagination-info">
        ...
        <span class="clientes-importador-row-hint">Clique em uma linha para ver as informaÃ§Ãµes completas.</span>
      </div>
      <div class="clientes-importador-pagination-controls" aria-label="PaginaÃ§Ã£o dos clientes importados">
        ...
        ${pageButtons}
        <button type="button" class="clientes-importador-page-btn" data-page="${currentPage + 1}" ${currentPage >= totalPages ? 'disabled' : ''}>PrÃ³xima</button>
        <button type="button" class="clientes-importador-page-btn" data-page="${totalPages}" ${currentPage >= totalPages ? 'disabled' : ''}>Ãšltima</button>
      </div>
    `;
  }
  ...
  function detailItem(label, value, wide = false) {
    ...
    return `
      <div class="clientes-importador-detail-item ${wide ? 'clientes-importador-detail-wide' : ''}">
        ...
        <span class="clientes-importador-detail-value ${empty ? 'empty' : ''}">${escapeHtml(empty ? 'NÃ£o informado' : value)}</span>
      </div>
    `;
  }
  ...
  function openInlineClientDetail(anchorRow, titleText, subtitleText, bodyHtml) {
    ...
    detailRow.innerHTML = `
      <td class="clientes-importador-inline-detail-cell" colspan="${FIELDS.length}">
        <div class="clientes-importador-detail-modal" role="dialog" aria-modal="false" aria-label="InformaÃ§Ãµes do Cliente">
          <div class="clientes-importador-detail-head">
            <div>
              <h3 class="clientes-importador-detail-title"></h3>
              <p class="clientes-importador-detail-subtitle"></p>
            </div>
            <button type="button" class="clientes-importador-close" aria-label="Fechar">Ã—</button>
          </div>
  ...
  function openClientDetail(row, anchorRow) {
    ...
    const documentoPrincipal = getDisplayValue(row, { key: 'cnpj' }) || getDisplayValue(row, { key: 'cpf' }) || 'Documento nÃ£o informado';

    title.textContent = fantasia;
    subtitle.textContent = `CÃ³digo ${codigo} â€¢ ${documentoPrincipal}`;

    body.innerHTML = `
      <div class="clientes-importador-detail-section">
        <h4>Dados principais</h4>
        <div class="clientes-importador-detail-grid">
          ${detailItem('CÃ³digo', row.codigo)}
          ${detailItem('Fantasia', row.fantasia)}
          ${detailItem('CNPJ', row.cnpj)}
          ${detailItem('CPF', row.cpf)}
          ${detailItem('Atividade', row.atividade)}
          ${detailItem('Empresa ResponsÃ¡vel', row.empresaResponsavel)}
        </div>
      </div>
      <div class="clientes-importador-detail-section">
        <h4>Contato e responsÃ¡veis</h4>
        <div class="clientes-importador-detail-grid">
      ...
      <div class="clientes-importador-detail-section">
        <h4>EndereÃ§o completo</h4>
        <div class="clientes-importador-detail-grid">
          ${detailItem('EndereÃ§o Completo', row.enderecoCompleto || buildEnderecoCompleto(row), true)}
          ${detailItem('Logradouro', row.logradouro)}
          ${detailItem('EndereÃ§o', row.endereco)}
          ${detailItem('NÃºmero', row.numero)}
  ...
  function handleFileSelected(event) {
    ...
    if (!file) return;
    if (!window.XLSX) {
      alert('Biblioteca de Excel nÃ£o carregada. Verifique sua conexÃ£o e tente novamente.');
      return;
    }
    const reader = new FileReader();
    reader.onload = function (loadEvent) {
      try {
        ...
        if (!firstSheetName) throw new Error('Arquivo sem planilha vÃ¡lida.');
        const sheet = workbook.Sheets[firstSheetName];
        const matrix = XLSX.utils.sheet_to_json(sheet, { header: 1, defval: '' });
        const cleaned = matrix.filter(row => Array.isArray(row) && row.some(cell => String(cell || '').trim() !== ''));
        if (cleaned.length < 2) throw new Error('A planilha precisa ter cabeÃ§alho e pelo menos uma linha de dados.');
  ...
  function validateMappedRows(rows) {
    ...
    IMPORT_FIELDS.filter(field => field.required).forEach(field => {
      if (!state.mapping[field.key]) {
        ...
        errors.push({ line: 'Mapeamento', field: field.label, message: `O campo obrigatÃ³rio "${field.label}" nÃ£o foi mapeado.` });
      }
    });

    if (!state.mapping.cnpj && !state.mapping.cpf) {
      hasMappingError = true;
      errors.push({ line: 'Mapeamento', field: 'CNPJ/CPF', message: 'Mapeie CNPJ ou CPF. Cliente sem CNPJ serÃ¡ aceito quando tiver CPF.' });
    }
    ...
    rows.forEach(row => {
      ...
      if (isEmptyValue(row.codigo)) rowErrors.push({ line: row.__line, field: 'CÃ³digo', message: 'CÃ³digo vazio.' });
      if (isEmptyValue(row.fantasia)) rowErrors.push({ line: row.__line, field: 'Fantasia', message: 'Fantasia vazia.' });
      if (!cnpjDigits && !cpfDigits) rowErrors.push({ line: row.__line, field: 'CNPJ/CPF', message: 'Informe CNPJ ou CPF. Cliente sem CNPJ Ã© aceito quando tiver CPF.' });
      if (!isEmptyValue(row.email) && !isValidEmail(row.email)) rowErrors.push({ line: row.__line, field: 'Email', message: 'Email invÃ¡lido.' });

      if (codeNorm) {
        if (existingCodes.has(codeNorm)) rowErrors.push({ line: row.__line, field: 'CÃ³digo', message: 'CÃ³digo jÃ¡ importado anteriormente.' });
        if (seenCodes.has(codeNorm)) rowErrors.push({ line: row.__line, field: 'CÃ³digo', message: 'CÃ³digo duplicado dentro da planilha.' });
        seenCodes.add(codeNorm);
      }

      if (cnpjDigits) {
        if (existingCnpjs.has(cnpjDigits)) rowErrors.push({ line: row.__line, field: 'CNPJ', message: 'CNPJ jÃ¡ importado anteriormente.' });
      ...
      if (cpfDigits) {
        if (existingCpfs.has(cpfDigits)) rowErrors.push({ line: row.__line, field: 'CPF', message: 'CPF jÃ¡ importado anteriormente.' });
  ...
  function renderPreview() {
    ...
    `).join('') || `<tr><td colspan="${IMPORT_FIELDS.length}" style="text-align:center;color:var(--text-muted);">Nenhuma linha para prÃ©via.</td></tr>`;
  }
  ...
  function renderErrors() {
    ...
    if (state.errors.length) {
      ...
      errorsBox.innerHTML = `<strong>${hasValid ? 'AtenÃ§Ã£o: as linhas abaixo serÃ£o ignoradas, mas as vÃ¡lidas podem ser importadas.' : 'Corrija antes de importar:'}</strong><ul style="margin:8px 0 0 18px;padding:0;">${state.errors.map(err => `<li>Linha ${escapeHtml(err.line)} â€” ${escapeHtml(err.field)}: ${escapeHtml(err.message)}</li>`).join('')}</ul>`;
      confirmBtn.disabled = !hasValid;
      confirmBtn.textContent = hasValid ? `Confirmar ImportaÃ§Ã£o (${state.validRows.length} vÃ¡lidos)` : 'Confirmar ImportaÃ§Ã£o';
      errorsBtn.style.display = 'inline-flex';
    } else {
      errorsBox.classList.remove('open');
      errorsBox.innerHTML = '';
      confirmBtn.disabled = state.mappedRows.length === 0;
      confirmBtn.textContent = state.mappedRows.length ? `Confirmar ImportaÃ§Ã£o (${state.mappedRows.length})` : 'Confirmar ImportaÃ§Ã£o';
  ...
  async function confirmImport() {
    if (!isAdminUser()) {
      showToast('Somente administrador pode confirmar importaÃ§Ã£o de clientes.');
      return;
    }
    updatePreviewAndValidation();
    const rowsForImport = state.validRows.length ? state.validRows : (state.errors.length ? [] : state.mappedRows);
    if (!rowsForImport.length) {
      showToast('Nenhuma linha vÃ¡lida para importar. Corrija a planilha ou o mapeamento.');
      return;
    }
    ...
    try {
      ...
      alert('NÃ£o foi possÃ­vel salvar no banco de dados. Verifique sua conexÃ£o/login e tente novamente. Erro: ' + (err.message || err));
      if (confirmBtn) {
        confirmBtn.disabled = false;
        confirmBtn.textContent = oldText || 'Confirmar ImportaÃ§Ã£o';
      }
    }
  }
  ...
  function exportRows(useFiltered) {
    ...
    const loggedUser = window.Store && Store.getLoggedUser ? Store.getLoggedUser() : null;
    const meta = [
      ['RELATÃ“RIO: CLIENTES IMPORTADOR DO SISTEMA'],
      ['Data da ExportaÃ§Ã£o:', new Date().toLocaleDateString('pt-BR') + ' ' + new Date().toLocaleTimeString('pt-BR')],
      ['UsuÃ¡rio ResponsÃ¡vel:', loggedUser ? `${loggedUser.name || ''} (${loggedUser.username || loggedUser.id || ''})` : 'Sistema'],
  ...
  // NÃ£o chama init a cada alteraÃ§Ã£o da tabela, evitando tela travada/congelada.
...
/*
 * IntegraÃ§Ã£o segura com Clientes Importador do Sistema
 * - Oculta o seletor global de vendedor do topo da barra lateral.
 * - Usa a base de clientes importados para autopreencher MovimentaÃ§Ã£o, Chamados e Simulador de Troca.
 * - Campos preenchidos automaticamente ficam somente leitura.
 * - Se o cÃ³digo nÃ£o existir, os campos do cliente permanecem editÃ¡veis para digitaÃ§Ã£o manual.
 */
(function () {
  'use strict';
  ...
  function injectStyle() {
    ...
    style.textContent = `
      /* Remove o seletor antigo de vendedor da barra lateral, sem apagar dados nem lÃ³gica interna. */
  ...
  async function ensureRowsLoaded(force = false) {
    ...
    try {
      ...
      console.warn('Clientes importados: nÃ£o foi possÃ­vel atualizar cache do banco.', err.message || err);
      return local;
    } finally {
  ...
  function getFixedCompanyText() {
    ...
    return unitName || user.empresa_name || user.company_name || user.empresa_id || 'Empresa do usuÃ¡rio';
  }

  function getFixedSellerName() {
    const user = getLoggedUser();
    return user.name || user.nome || user.username || 'UsuÃ¡rio logado';
  }
  ...
  async function handleMovementClientCode() {
    ...
    if (!code) {
      ...
      setHint(input, 'mov-client-id-hint', '', 'Digite o cÃ³digo para buscar na base de clientes importados. Se nÃ£o encontrar, preencha manualmente.');
      return;
    }
    ...
    if (!row) {
      ...
      const sellerManual = document.getElementById('mov-client-seller');
      if (sellerManual) { delete sellerManual.dataset.ccImportedLocked; }
      setHint(input, 'mov-client-id-hint', 'not-found', 'CÃ³digo nÃ£o encontrado. VocÃª pode preencher os dados do cliente manualmente.');
      return;
    }
    ...
    setHint(input, 'mov-client-id-hint', 'found', 'Cliente localizado na base importada. Os dados automÃ¡ticos ficaram bloqueados.');
  }
  ...
  function bindMovementAutofill() {
    ...
    setHint(input, 'mov-client-id-hint', '', 'Digite o cÃ³digo para buscar na base de clientes importados.');
    fixMovementIdentityFields();
  }
  ...
  function ensureTicketClientCodeField() {
    ...
    codeGroup.innerHTML = `
      <label for="ticket-open-client-code">CÃ³digo do Cliente</label>
      <input type="text" id="ticket-open-client-code" placeholder="Digite o cÃ³digo, se houver...">
    `;
  ...
  async function handleTicketClientCode() {
    ...
    if (!code) {
      ...
      setHint(input, 'ticket-open-client-code-hint', '', 'Digite o cÃ³digo para buscar na base importada. Se nÃ£o encontrar, preencha manualmente.');
      return;
    }
    ...
    if (!row) {
      ...
      setHiddenValue('ticket-open-client-seller-imported', '');
      setHint(input, 'ticket-open-client-code-hint', 'not-found', 'CÃ³digo nÃ£o encontrado. VocÃª pode preencher os dados do cliente manualmente.');
      return;
    }
    ...
    setHint(input, 'ticket-open-client-code-hint', 'found', 'Cliente localizado na base importada. Os dados automÃ¡ticos ficaram bloqueados.');
  }
  ...
  function bindTicketAutofill() {
    ...
    // Se o patrimÃ´nio preencher algum dado pelo histÃ³rico, mantÃ©m unidade e vendedor travados no usuÃ¡rio logado.
    ...
    setHint(input, 'ticket-open-client-code-hint', '', 'Digite o cÃ³digo para buscar na base de clientes importados.');
    fixTicketIdentityFields();
  }
  ...
  async function handleExchangeClientCode() {
    ...
    if (!code) {
      ...
      setHint(input, 'exchange-client-code-hint', '', 'Digite o cÃ³digo para buscar o nome fantasia automaticamente.');
      return;
    }
    ...
    if (!row) {
      setLocked(nameInput, false);
      setHiddenValue('exchange-client-seller-imported', '');
      setHint(input, 'exchange-client-code-hint', 'not-found', 'CÃ³digo nÃ£o encontrado. VocÃª pode digitar o nome fantasia normalmente.');
      return;
    }
  ...
  function bindExchangeAutofill() {
    ...
    setHint(input, 'exchange-client-code-hint', '', 'Digite o cÃ³digo para buscar na base de clientes importados.');
  }
  ...
  function bindSubmitSafeguards() {
    ...
    if (movementForm && movementForm.dataset.ccSubmitSafeguardBound !== '1') {
      ...
      movementForm.addEventListener('submit', () => {
        // Garante que campos fixos/automÃ¡ticos tenham valor antes do envio, inclusive em navegadores mobile/PWA.
        fixMovementIdentityFields();
  ...
  // Reaplica periodicamente porque algumas telas recarregam formulÃ¡rios apÃ³s buscar dados da API.
  setInterval(() => {
...
// Central de Tutoriais - busca e inicializaÃ§Ã£o isolada
(function(){
  function norm(text) {
...
/*
  CorreÃ§Ã£o controlada - clientes cadastrados globais + fila de aprovaÃ§Ã£o restrita.
  NÃ£o altera layout geral. Apenas garante que cadastros feitos por qualquer usuÃ¡rio
  apareÃ§am para todos e que a aprovaÃ§Ã£o continue restrita.
*/
(function(){
  'use strict';
  ...
  function canApproveClients(user) {
    ...
    // NÃ£o liberar fila por permissÃ£o simples de Clientes/Cadastro.
  ...
  function installStorePatch() {
    ...
    Store.deleteClient = async function(id) {
      ...
      // Remove tambÃ©m caches antigos por usuÃ¡rio para o registro nÃ£o voltar na prÃ³xima sincronizaÃ§Ã£o.
      for (let i = 0; i < localStorage.length; i += 1) {
        const key = localStorage.key(i);
  ...
  function installUiPatch() {
    ...
    UI.renderClients = function(clients) {
      ...
      body.innerHTML = list.map(c => {
        ...
        return `<tr class="cliente-compact-row mobile-summary-row" onclick="App.showClientDetails('${esc(c.id)}')"><td colspan="10"><div class="cliente-compact-card"><div class="cliente-compact-main"><div class="cliente-compact-name">${esc(c.name || c.nomeFantasia || c.companyName || 'Cliente sem nome')}</div><div class="cliente-compact-meta">${esc(sellerName(c.userId))} â€¢ ${esc(c.date || c.data_cadastro || c.created_at || c.createdAt || '-')} â€¢ ${scoreText(c)}</div></div><div class="cliente-compact-actions"><button class="btn btn-primary btn-xs" onclick="event.stopPropagation(); App.showClientDetails('${esc(c.id)}')">Ver Ficha</button>${adminBtn}</div></div></td></tr>`;
      }).join('') || `<tr><td colspan="10" style="text-align:center;color:var(--text-muted);padding:12px;">Nenhum cliente cadastrado.</td></tr>`;
    };
    ...
    UI.renderApprovals = function(clients) {
      ...
      const allowed = canApproveClients(user);
      if (!allowed) {
        body.innerHTML = `<tr><td colspan="9" style="text-align:center;color:var(--text-muted);padding:14px;">Fila de aprovaÃ§Ã£o restrita ao administrador ou responsÃ¡veis por equipamentos.</td></tr>`;
        return;
      }
      let list = Array.isArray(clients) ? clients.slice() : (Store.getAllClients ? Store.getAllClients() : []);
      const activeUnitId = Store.getActiveUnitId && Store.getActiveUnitId();
      ...
      if (!pending.length) {
        body.innerHTML = `<tr><td colspan="9" style="text-align:center;color:var(--text-muted);padding:14px;">Nenhum cadastro pendente de aprovaÃ§Ã£o.</td></tr>`;
        return;
      }
  ...
  function installAppPatch() {
    ...
    App.approveClient = function(id, newStatus) {
      const user = Store.getLoggedUser && Store.getLoggedUser();
      if (!canApproveClients(user)) {
        alert('Somente administrador ou responsÃ¡vel por equipamentos pode aprovar/reprovar clientes.');
        return;
      }
...
/*
  Ajuste controlado 02/07 - fluxo de aprovaÃ§Ã£o de clientes.
  Escopo: permissÃµes da fila, aprovaÃ§Ã£o/reprovaÃ§Ã£o, retorno para correÃ§Ã£o e visibilidade segura.
  NÃ£o altera banco diretamente nem muda aparÃªncia geral; trabalha sobre as funÃ§Ãµes existentes.
*/
(function(){
  'use strict';
  ...
  function canApproveClients(user){
    ...
    if (profile.includes('admin') || profile.includes('administrador')) return true;
    if (profile.includes('responsavel') && profile.includes('equip')) return true;
    // Regra 02/07: somente admin, responsÃ¡vel de equipamentos ou permissÃ£o explÃ­cita de liberaÃ§Ã£o/movimentaÃ§Ã£o.
    // PermissÃ£o apenas de "Clientes"/"Cadastro de Clientes" NÃƒO libera fila de aprovaÃ§Ã£o.
    return [
  ...
  async function saveClientsRemote(list){
    ...
    if (window.Store && Store.backendRequest && Store.getToken && Store.getToken()) {
      try {
        ...
        console.warn('Cadastro atualizado localmente; falha temporÃ¡ria ao sincronizar no banco:', err.message || err);
        throw err;
      }
    }
  }
  ...
  function sellerName(id, client){
    ...
    try {
      ...
      if (resolved && resolved !== 'UsuÃ¡rio nÃ£o localizado') return resolved;
    } catch (_) {}
    return direct || id || '-';
  }
  ...
  function renderClientsSafe(inputClients){
    ...
    body.innerHTML = list.map(c => {
      ...
      return `<tr class="mobile-summary-row" onclick="App.showClientDetails('${esc(c.id)}')">
        <td data-label="Cliente" style="font-weight:600;">${esc(c.name || c.nomeFantasia || c.companyName || '-')}<div class="mobile-only-subtext" style="font-size:.75rem;color:var(--text-muted);font-weight:normal;margin-top:4px;">${esc(c.city || '')} ${c.date ? 'â€¢ ' + esc(c.date) : ''}</div></td>
        <td data-label="CNPJ" class="mobile-hide">${esc(c.cnpj || '-')}</td>
        <td data-label="Categoria" class="mobile-hide">${esc(c.category || c.categoria || 'NÃ£o definida')}</td>
        ...
        <td data-label="AÃ§Ãµes"><button class="btn btn-primary btn-sm" style="padding:2px 8px;font-size:.75rem;" onclick="event.stopPropagation(); App.showClientDetails('${esc(c.id)}')">Ver Ficha</button>${actionCorrection}${delBtn}</td>
      </tr>`;
    }).join('') || `<tr><td colspan="10" style="text-align:center;color:var(--text-muted);padding:12px;">Nenhum cliente cadastrado.</td></tr>`;
  }
  ...
  function renderApprovalsSafe(inputClients){
    ...
    if (!list.length) {
      const msg = approver ? 'Nenhum cadastro pendente de aprovaÃ§Ã£o.' : 'Nenhum cadastro seu aguardando aprovaÃ§Ã£o ou correÃ§Ã£o.';
    ...
    body.innerHTML = list.map(c => {
      ...
      return `<tr class="mobile-summary-row" onclick="App.showClientDetails('${esc(c.id)}')">
        ...
        <td data-label="Status">${statusBadge(c)}</td>
        <td data-label="AÃ§Ãµes">${actions}</td>
      </tr>`;
    }).join('');
  }
  ...
  async function uploadOptionalPhotos(existingClient){
    ...
    for (const suffix of suffixes) {
      ...
      try {
        ...
        console.warn('Falha ao atualizar foto do cadastro em correÃ§Ã£o:', suffix, err.message || err);
      }
    }
    return result;
  }
  ...
  async function submitCorrection(e){
    ...
    if (idx < 0) return alert('Cadastro para correÃ§Ã£o nÃ£o encontrado.');
    ...
    if (isApproved(previous) && !canApproveClients(user)) return alert('Cadastro aprovado nÃ£o pode ser alterado por aqui.');
    ...
    try {
      ...
      if (window.App && App.refreshAllLists) App.refreshAllLists();
      showToast('Cadastro corrigido e reenviado para aprovaÃ§Ã£o!');
    } catch (err) {
      alert('NÃ£o foi possÃ­vel reenviar a correÃ§Ã£o agora: ' + (err.message || err));
    } finally {
      if (submitBtn) { submitBtn.disabled = false; submitBtn.textContent = 'Enviar Cadastro Completo para AprovaÃ§Ã£o'; }
    }
  }
  ...
  async function updateClientApprovalOnServer(id, status, reason, sendToCorrection){
    ...
    if (window.Store && Store.backendRequest && Store.getToken && Store.getToken()) {
      try {
        ...
        // Compatibilidade com Render ainda nÃ£o atualizado: usa a rota antiga /api/store/clients.
  ...
  function patchApp(){
    ...
    App.approveClient = async function(id, newStatus){
      const user = currentUser();
      if (!canApproveClients(user)) return alert('Somente administrador ou usuÃ¡rio autorizado para liberaÃ§Ã£o/movimentaÃ§Ã£o de equipamentos pode aprovar ou reprovar clientes.');
      if (newStatus === 'Reprovado') {
        const modal = document.getElementById('modal-rejection-reason');
        const form = document.getElementById('modal-rejection-form');
        if (!modal || !form) return alert('Modal de reprovaÃ§Ã£o nÃ£o encontrado.');
      ...
      try {
        ...
        alert('NÃ£o foi possÃ­vel salvar a aprovaÃ§Ã£o no banco: ' + (err.message || err));
      }
    };
    ...
    App.editClientCorrection = function(id){
      ...
      if (!client) return alert('Cadastro nÃ£o encontrado.');
      ...
      if (!(norm(client.status).includes('ajuste') || norm(client.status).includes('correc') || norm(client.status).includes('reprov'))) {
        return alert('Este cadastro nÃ£o estÃ¡ marcado para correÃ§Ã£o.');
      }
      window.location.hash = '#clientes';
      setTimeout(() => {
        ...
        const form = document.getElementById('client-form');
        if (!container || !form) return alert('FormulÃ¡rio de cliente nÃ£o encontrado.');
        container.classList.remove('hidden');
        ...
        if (!document.getElementById('client-correction-alert')) {
          ...
          box.innerHTML = `<strong>Cadastro em correÃ§Ã£o:</strong> ajuste as informaÃ§Ãµes necessÃ¡rias e reenvie para aprovaÃ§Ã£o. Motivo: ${esc(client.rejectionReason || '-')}`;
          form.prepend(box);
        }
        const fill = () => {
  ...
  function patchRejectionForm(){
    ...
    form.addEventListener('submit', async function(e){
      ...
      e.stopImmediatePropagation();
      const user = currentUser();
      if (!canApproveClients(user)) return alert('Sem permissÃ£o para aprovar/reprovar clientes.');
      const id = form.dataset.targetId;
      const reason = document.getElementById('modal-rejection-select')?.value || 'CorreÃ§Ã£o necessÃ¡ria';
      ...
      const fullReason = reason + (notes ? ' â€” ' + notes : '');
      try {
        ...
        form.reset();
        if (window.App && App.refreshAllLists) App.refreshAllLists();
        showToast(sendToCorrection ? 'Cadastro enviado para correÃ§Ã£o do vendedor e salvo no banco!' : 'Cadastro reprovado e salvo no banco!');
      } catch (err) {
        alert('NÃ£o foi possÃ­vel salvar a reprovaÃ§Ã£o/correÃ§Ã£o no banco: ' + (err.message || err));
      }
    }, true);
  }
    ...
    UI.showClientDetails = function(client){
      ...
      setTimeout(() => {
        ...
        if (canCorrect && !document.getElementById('btn-client-correction-from-details')) {
          ...
          btn.textContent = 'Corrigir cadastro e reenviar para aprovaÃ§Ã£o';
...
/*
  Ajuste final 02/07 - sessÃ£o Ãºnica, visibilidade de clientes pendentes e correÃ§Ã£o por notificaÃ§Ã£o.
  Escopo controlado: nÃ£o altera layout geral nem banco, apenas endurece regras de exibiÃ§Ã£o/fluxo.
*/
(function(){
  'use strict';
  ...
  function canApproveClients(u){
    ...
    if (profile.includes('admin') || profile.includes('administrador')) return true;
    if (profile.includes('responsavel') && profile.includes('equip')) return true;
    // PermissÃ£o "Clientes" ou "Cadastro de Clientes" nÃ£o deve abrir Fila de AprovaÃ§Ãµes.
    return [
      'aprovacao de clientes',
      'aprovar clientes',
      'liberacao de cadastro de clientes',
  ...
  function injectCorrectionPhotoPreview(client){
    ...
    box.innerHTML = `<strong style="display:block;color:var(--primary-light);margin-bottom:8px;">Fotos atuais do cadastro</strong>` +
      (photos.length ? `<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(110px,1fr));gap:10px;">${photos.map(([label,src]) => `<div style="border:1px solid var(--border-color);border-radius:8px;padding:6px;background:rgba(15,23,42,.55);"><div style="font-size:.72rem;color:var(--text-muted);margin-bottom:5px;">${esc(label)}</div><img src="${esc(src)}" alt="${esc(label)}" style="width:100%;height:80px;object-fit:cover;border-radius:6px;"></div>`).join('')}</div><p style="font-size:.72rem;color:var(--text-muted);margin-top:8px;">Se nÃ£o escolher uma nova foto, a foto atual serÃ¡ mantida.</p>` : `<p style="font-size:.78rem;color:var(--text-muted);margin:0;">Este cadastro nÃ£o possui fotos salvas ou elas ainda nÃ£o sincronizaram. Ao reenviar, novas fotos podem ser anexadas normalmente.</p>`);
  ...
  function patchCorrectionOpen(){
    ...
    App.openClientCorrectionFromNotification = async function(notificationId, clientId){
      ...
      if (!client) return alert('NÃ£o encontrei este cadastro para correÃ§Ã£o. Atualize a tela e tente novamente.');
      if (!isOwner(client, user()) && !canApproveClients(user())) return alert('Somente o vendedor que cadastrou pode corrigir este cadastro.');
  ...
  function patchRejectionModalLabels(){
    ...
    if (title) title.textContent = 'Reprovar ou devolver cadastro para correÃ§Ã£o';
    const help = document.querySelector('#modal-rejection-reason p');
    if (help) help.textContent = 'Informe o motivo. Marcando correÃ§Ã£o, o vendedor receberÃ¡ uma notificaÃ§Ã£o e poderÃ¡ editar o formulÃ¡rio preenchido para reenviar.';
    const check = document.getElementById('modal-rejection-send-to-correction');
...
(function(){
  ...
  function userName(id){ try { var n = UI.getUserName ? UI.getUserName(id) : id; return n === 'UsuÃ¡rio nÃ£o localizado' ? id : n; } catch(_) { return id || '-'; } }
  ...
  function ensureAdicaoCodeField(){ var model = document.getElementById('mov-modelo-adicao'); if (!model || document.getElementById('mov-patrimonio-adicao')) return; var group = document.createElement('div'); group.className = 'form-group'; group.innerHTML = '<label for="mov-patrimonio-adicao">NÃºmero do PatrimÃ´nio</label><input type="text" id="mov-patrimonio-adicao" placeholder="Digite o nÃºmero do patrimÃ´nio..."><div id="hist-patrimonio-adicao-link" style="margin-top:4px;display:none;"></div>'; model.closest('.form-group')?.insertAdjacentElement('beforebegin', group); }
...
/* OtimizaÃ§Ã£o de listas pesadas: paginaÃ§Ã£o local e despesas sem carregar miniaturas. */
(function(){
  'use strict';
  ...
  const TARGETS = {
    ...
    movimentacao: { renderMethod: 'renderMovements', tbodyId: 'movements-table-body', storeMethod: 'getMovements', label: 'movimentaÃ§Ãµes' },
  ...
  function pending(status){
    return ['pendente','aguardando','aguardando aprovacao','aguardando aprovaÃ§Ã£o'].includes(normalize(status));
  }
  ...
  function attachmentBadge(exp){
    ...
    return `<span class="badge-status badge-primary" style="font-size:.7rem;font-weight:600;">ðŸ“Ž ${total} anexo${total > 1 ? 's' : ''}</span>`;
  }
  function actionButtonsExpense(exp){
    ...
    if (approvalMode && pending(exp && exp.status)) {
      return `<div style="display:flex;gap:4px;justify-content:center;flex-wrap:wrap;">
        ...
        <button class="btn btn-secondary btn-sm" onclick="event.stopPropagation(); App.correctRegisteredExpense && App.correctRegisteredExpense('${id}')">CorreÃ§Ã£o</button>
      </div>`;
    }
    let correction = '';
    const user = getUser();
  ...
  function renderPagination(moduleKey, total, start, end){
    ...
    const text = total ? `Mostrando ${start + 1}â€“${end} de ${total} ${cfg.label}` : `Nenhum registro para mostrar.`;
    pager.innerHTML = `<div class="cc-pager-info">${esc(text)}</div>
      <div class="cc-pager-actions">
        ...
        <span class="cc-pager-page">PÃ¡gina ${page} de ${totalPages}</span>
        <button type="button" class="btn btn-secondary btn-sm" ${disableNext} onclick="window.__ccPagerGo('${moduleKey}', ${page + 1})">PrÃ³xima</button>
      </div>`;
  }
  ...
  function renderExpensesLight(raw){
    ...
    body.innerHTML = pageItems.map(exp => {
      ...
      return `<tr class="mobile-summary-row" onclick="window.__ccOpenExpenseDetails('${id}')">
        ...
        <td data-label="OperaÃ§Ã£o">${esc(exp.operacao || '-')}</td>
        ...
        <td data-label="ResponsÃ¡vel"><span style="font-size:.75rem;color:var(--text-muted);">${esc(expenseUser(exp))}</span></td>
        ...
        <td data-label="AÃ§Ãµes">${actionButtonsExpense(exp)}</td>
      </tr>`;
    }).join('');
  ...
  function renderWithBase(moduleKey, raw){
    ...
    if (!pageItems.length) {
      ...
      const msg = moduleKey === 'chamados' ? 'Nenhum chamado mecÃ¢nico encontrado.' : 
                  moduleKey === 'clientes' ? 'Nenhum cliente cadastrado.' : 'Nenhuma movimentaÃ§Ã£o registrada.';
...
(function() {
  ...
  function showInAppCamera() {
    ...
    if (!cameraOverlay) {
      ...
      cameraOverlay.innerHTML = `
        <div style="width: 100%; display: flex; justify-content: space-between; align-items: center; padding: 15px; box-sizing: border-box; background: rgba(0,0,0,0.5); position: absolute; top:0; z-index: 10;">
          <span style="color:#fff; font-size:1.1rem; font-weight:600;">CÃ¢mera do Sistema</span>
    ...
    navigator.mediaDevices.getUserMedia({
      ...
      alert('NÃ£o foi possÃ­vel acessar a cÃ¢mera. Verifique se deu permissÃ£o de acesso Ã  cÃ¢mera no seu navegador.');
      closeInAppCamera();
    });
  }
...
(function(){
  ...
  function addAdminTicketButtons(){
    ...
    document.querySelectorAll('#tickets-table-body tr').forEach(function(tr){
      ...
      var action = tr.querySelector('td[data-label="AÃƒÂ§ÃƒÂ£o"],td[data-label="AÃ§Ã£o"]');
  ...
  function patchTicketDetails(){
    ...
    App.showTicketDetails = function(id){
      ...
      setTimeout(function(){
        ...
        var html = '<div data-client-code-row style="margin:8px 0;padding:8px;border:1px solid var(--border-color);border-radius:6px;"><span style="display:block;color:var(--text-muted);font-size:.7rem;font-weight:700;text-transform:uppercase;">CÃƒÂ³digo do Cliente</span><strong>'+esc(code)+'</strong></div>';
  ...
  function patchMovementSubmit(){
    document.addEventListener('submit', async function(ev){
      ...
      ev.preventDefault();
      ev.stopImmediatePropagation();
      if (!isAdmin()) return alert('Somente administrador pode editar movimentaÃƒÂ§ÃƒÂµes.');
      var id = form.dataset.editingId;
      ...
      try {
        ...
        App.showToast('MovimentaÃƒÂ§ÃƒÂ£o editada com sucesso.');
      } catch(err) {
        alert('Erro ao editar movimentaÃƒÂ§ÃƒÂ£o: ' + (err.message || err));
      }
    }, true);
  }
  ...
  function patchClientSubmit(){
    document.addEventListener('submit', async function(ev){
      ...
      if (idx < 0) return alert('Cliente nÃƒÂ£o encontrado.');
  ...
  function installAppEditors(){
    ...
    App.editTicketAdmin = function(id){
      ...
      if (!t) return alert('Chamado nÃƒÂ£o encontrado.');
      normalizeTicketTypeSelect();
    ...
    App.editMovementAdmin = async function(id){
      ...
      if (!mov) return alert('MovimentaÃƒÂ§ÃƒÂ£o nÃƒÂ£o encontrada.');
    ...
    App.editClientAdmin = function(id){
      ...
      if (!c) return alert('Cliente nÃƒÂ£o encontrado.');
