    ...
    if (lead) {
      if (confirm(`Deseja converter o lead "${lead.name}" em cliente e iniciar o processo de cadastro?`)) {
        ...
        setTimeout(() => {
          ...
          this.showToast('Dados da prospecção importados! Preencha o CNPJ e envie para aprovação.');
        }, 150);
      } else {
        // Reset select values
        this.refreshAllLists();
...
App.addExchangeItemFromModal = function() {
  ...
  const qty = parseInt(document.getElementById('exchange-item-qty').value, 10) || 0;
